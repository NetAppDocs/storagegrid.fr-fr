...
---
= Surveillez les opérations de vérification des objets dans StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
image:nodes_storage_node_object_verification.png["Nœuds > Nœud de stockage > Objet > Vérification"]
...
+
image:nodes_storage_node_ilm_ec_verification.png["Nœuds > Nœud de stockage > ILM > Vérification EC"]

+
Sélectionnez le point d'interrogation image:icon_nms_question.png["point d'interrogation"] à côté du nom d'un attribut pour afficher le texte d'aide.
...
+
image:oec_job_history.png["Historique du travail de vérification de l'existence d'un objet"]
