...
La version du logiciel SANtricity est inférieure au minimum recommandé pour cette version de StorageGRID.
 ...
 a| 
Défaut SFP détecté sur l'appareil
 a| 
Une panne SFP a été détectée dans le contrôleur de calcul d'une appliance StorageGRID.
...
La gestion des versions ou le verrouillage d'objet S3 d'un compartiment FabricPool est activé, ce qui n'est pas pris en charge.
 ...
 a| 
ListBucket vérification des autorisations pour Get/Head object désactivée
 a| 
ListBucket la vérification des autorisations pour GetObject/HeadObject sur des clés inexistantes est désactivée.
...
| Nom de l'alerte | Description 
 ...
 a| 
Nombre élevé de goroutines du service de chunk
 a| 
Le service de chunk présente un nombre anormalement élevé de goroutines, ce qui pourrait indiquer un deadlock ou une fuite de ressources.
...
Les fragments avec code d'effacement ne peuvent plus être vérifiés. Des fragments corrompus peuvent ne pas être réparés.
 ...
 a| 
Groupes à codage d'effacement en état Needs Repair
 a| 
Un ou plusieurs groupes codés par effacement sont dans l’état « Nécessite une réparation » alors qu’aucune réparation de données codées par effacement n’est en cours.
...
Des objets orphelins ont été détectés.
 ...
 a| 
La taille de la partie de l’envoi multipart S3 est trop grande
 a| 
Un client tente une opération de téléchargement en plusieurs parties qui dépasse la limite de taille des parties.
...
Un client tente une opération PUT Object qui dépasse les limites de taille S3.
 ...
 a| 
Requêtes S3 utilisant des en-têtes conditionnels non pris en charge
 a| 
Un client S3 utilise des en-têtes conditionnels non pris en charge (If-Match ou If-None-Match) pour certaines opérations.



 a| 
Requêtes S3 utilisant une région incorrecte avec l'espace de noms global activé
 a| 
Un client S3 utilise la mauvaise région de signature pour accéder aux compartiments de cette grille.
