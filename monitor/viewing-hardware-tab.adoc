...
---
= Consultez l'onglet Matériel StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
image:nodes_page_hardware_tab_graphs.png["Onglet matériel de la page nœuds"]
...
image:nodes_page_memory_usage_details.png["Page des nœuds > Matériel > Détails d'utilisation de la mémoire"]
...
+
image:nodes_page_overview_tab_extended.png["Vue d'ensemble de la page nœuds étendue"]
...
+
.. Consultez les graphiques d'utilisation du processeur et de la mémoire pour connaître les pourcentages d'utilisation du processeur et de la mémoire au fil du temps. Pour afficher un intervalle de temps différent, sélectionnez l'une des commandes situées au-dessus du graphique. Vous pouvez afficher les informations pour des intervalles de 1 heure, 1 jour, 1 semaine ou 1 mois. Vous pouvez également définir un intervalle personnalisé, ce qui vous permet de spécifier des plages de dates et d'heures.
+
image:nodes_page_hardware_tab_graphs.png["Graphiques de matériel"]
...
État de température du contrôleur de calcul.
 ...
 a| 
Alimentation du contrôleur de calcul A
 a| 
État de l'alimentation A du contrôleur de calcul.



 a| 
Alimentation du contrôleur de calcul B
 a| 
État de l'alimentation B du contrôleur de calcul.
...
+
image:nodes_page_network_traffic_graph.png["Courbes de trafic réseau de la page noeuds"]

+
.. Consultez la section Interfaces réseau.
+
image:nodes_page_network_interfaces.png["Nœuds page interfaces réseau"]

+
Utilisez le tableau suivant avec les valeurs de la colonne *Vitesse* du tableau des interfaces réseau pour déterminer si les ports réseau 10/25-GbE de l'appliance ont été configurés pour utiliser le mode actif/de secours ou le mode LACP.
...
.. Consultez la section sur la communication réseau.
+
...
+
image:nodes_page_storage_used_object_data.png["Stockage utilisé : données d'objet"]

+
image:storage_used_object_metadata.png["Stockage utilisé : métadonnées d'objet"]
...
+
image:nodes_page_overview_tab_services_appliance.png["Onglet de présentation de la page nœuds pour l'appliance de services"]
...
. Sélectionnez l'onglet *Matériel* pour obtenir plus d'informations sur l'appareil.
+
...
+
image:nodes_page_hardware_tab_graphs_services_appliance.png["Page noeuds onglet matériel graphiques de l'appareil de services"]

.. Faites défiler vers le bas pour consulter le tableau des composants de l'appareil. Ce tableau contient des informations telles que le nom du modèle, le numéro de série, la version du micrologiciel du contrôleur et l'état de chaque composant.
+
[cols="1a,2a"]
|===
| Dans la table Appliance | Description 
...
. Sélectionnez l'onglet *Réseau* pour afficher les informations de chaque réseau.
+
Le graphique trafic réseau fournit un récapitulatif du trafic réseau global.
...
.. Consultez la section Interfaces réseau.
+
Utilisez le tableau suivant avec les valeurs de la colonne *Vitesse* du tableau des interfaces réseau pour déterminer si les quatre ports réseau 40/100-GbE de l'appliance ont été configurés pour utiliser le mode actif/de secours ou le mode LACP.
...
. Sélectionnez l'onglet *Stockage* pour afficher les informations relatives aux périphériques de disque et aux volumes de l'appliance de services.
