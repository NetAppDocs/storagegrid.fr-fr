...
---
= Surveillez l'activité des locataires dans StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
image:tenant_accounts_page.png["Page comptes de tenant"]

. Vous pouvez également vous connecter à un compte locataire en sélectionnant le lien Sign in image:icon_tenant_sign_in.png["Icône de connexion du locataire"] dans la colonne *Sign in/Copy URL*.
. Vous pouvez également copier l'URL de la page de Sign in d'un locataire en sélectionnant le lien de copie d'URL image:icon_tenant_copy_url.png["Icône URL de copie du locataire"] dans la colonne *Sign in/Copier l'URL*.
...
+
image:tenant_accounts_example_csv.png["Exemple de comptes de locataires .csv"]
...
+
image:tenant_usage_modal.png["Modal d'utilisation des locataires"]
...
+
image:tenant_bucket_space_consumption_GM.png["Tableau à barres de consommation du compartiment des locataires"]
...
+
image:tenant_bucket_details_csv.png["Détails du compartiment locataire exemple CSV"]
