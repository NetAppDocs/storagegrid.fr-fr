...
---
= Collectez les fichiers journaux et les données système de StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Si le nœud d'administration utilisé pour lancer la collecte des journaux est mis hors ligne pendant celle-ci, StorageGRID ne conserve pas les résultats. Vous devez redémarrer le nœud et relancer la collecte des journaux une fois le redémarrage terminé.
...
NOTE: Vous pouvez choisir de laisser le champ « phrase secrète de provisionnement » vide pour une nouvelle tentative standard, ou vous pouvez saisir la phrase secrète pour réessayer via SSH.
