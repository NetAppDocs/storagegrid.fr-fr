...
[role="lead"]
Configurez, selon les besoins, les niveaux d'audit, les en-têtes de protocole, l'emplacement des messages et des journaux d'audit, ainsi que la durée de conservation des journaux.
...
Vous pouvez, si vous le souhaitez, link:../monitor/considerations-for-external-syslog-server.html["configurer un serveur syslog externe"] enregistrer les informations d'audit à distance. Vous pouvez minimiser l'impact sur les performances de la journalisation des messages d'audit sans réduire l'exhaustivité des données d'audit en utilisant un serveur externe. Un serveur syslog externe est particulièrement utile si vous disposez d'une grande grille, utilisez plusieurs types d'applications S3 ou souhaitez conserver toutes les données d'audit.
...
+
Lorsque StorageGRID détecte des en-têtes HTTP dans une requête, il les inclut dans le message d'audit sous le champ HTRH.
...
Par défaut, les messages d'audit et les journaux sont enregistrés sur les nœuds où ils sont générés. StorageGRID effectue une rotation régulière et supprime finalement les messages d'audit et les journaux afin d'éviter qu'ils ne consomment un espace disque excessif. Pour enregistrer les messages d'audit et un sous-ensemble des journaux en externe, <<use-external-syslog-server,utiliser un serveur syslog externe>>.
...
+
Si vous sélectionnez une valeur, StorageGRID l'applique à tous les messages de ce type. Vous perdez les informations relatives aux différents niveaux de gravité si vous remplacez un niveau de gravité par une valeur fixe.
...
Avant d'utiliser un serveur syslog externe, demandez à tous les nœuds de votre grille d'envoyer des messages de test au serveur syslog externe. Ces messages de test vous aident à valider l'ensemble de votre infrastructure de collecte de journaux avant de vous engager à envoyer des données au serveur syslog externe.
...
== Gérer la conservation des journaux

Vous pouvez gérer la durée pendant laquelle StorageGRID conserve les journaux en créant une règle ILM ou en configurant une politique de cycle de vie de compartiment S3.

[role="tabbed-block"]
====
.Règle ILM
--
Créez une règle ILM pour le compartiment et ajoutez cette règle à la stratégie ILM active.

.Étapes
. Sélectionnez *ILM* > *règles*.
. Sélectionnez *Créer*. link:../ilm/create-ilm-rule-enter-details.html["Étape 1 (entrer les détails)"] de l’assistant Créer une règle ILM s’affiche :
+
.. Saisissez un nom unique pour la règle. Par exemple : Règle ILM pour les journaux archivés.
.. Sélectionnez le compte locataire S3 d'archivage des journaux.
.. Spécifiez le compartiment S3 d'archivage des journaux.


. Continuez à link:../ilm/create-ilm-rule-define-placements.html["Étape 2 (définir les placements)"] et définissez l'emplacement pour la règle des journaux archivés. Par exemple, la règle ILM stocke une copie répliquée sur le site 1 et une copie répliquée sur le site 2 pendant la première année. Après un an, une copie à codage d'effacement 2+1 est créée et enregistrée sur un seul site.
. Après avoir créé la règle, ajoutez-la à une stratégie ILM active.


--
.cycle de vie d'un compartiment S3
--
Configurez une stratégie de cycle de vie S3 sur le compartiment S3 d'archivage des journaux :
...
----
Lorsque vous configurez une stratégie de cycle de vie pour un compartiment S3 d'archivage des journaux, les actions d'expiration du cycle de vie prévalent sur la stratégie ILM pour les objets correspondant au filtre de cycle de vie du compartiment. Les objets qui ne correspondent pas au filtre de cycle de vie du compartiment utilisent les paramètres de rétention de la stratégie ILM. Si un objet correspond à un filtre de cycle de vie du compartiment et qu'aucune action d'expiration n'est explicitement spécifiée, les paramètres de rétention de la stratégie ILM ne sont pas utilisés et il est sous-entendu que les versions de l'objet sont conservées indéfiniment.
