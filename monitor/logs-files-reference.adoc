...
| acct-tunnel.log  a| 
Journal de l’assistant de tunnel de compte qui fonctionne avec le répartiteur de charge.
...
Journal du trafic du proxy HTTP interne.
...
| manage-audit.log  a| 
`mgmt-api` accéder aux journaux d'audit.
...
* stream-access.log : sessions proxy de flux TCP/TLS (couche 4) pour le bloc de flux.
+
*Remarque* : Ceci n’est pas un journal de requêtes HTTP.
...
Journal d'accès pour le trafic de fédération de grille acheminé comme S3 (espace de noms global/URL de fédération).
...
| system-bucket.log  a| 
`update-system-bucket-configuration.log` : Journaux du compartiment système.
...
`update-cors-configurations.log`: Journaux de traces relatifs aux modifications TLS/chiffrements et CORS.



| update-leakd-configurations.log  a| 
Journal de configuration de la détection des orphelins.
