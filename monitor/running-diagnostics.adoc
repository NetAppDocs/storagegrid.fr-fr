...
---
= Exécutez des diagnostics dans StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* image:icon_alert_green_checkmark.png["Icône alerte verte coche"] *Normal* : Toutes les valeurs se situent dans la plage normale.
* image:icon_alert_yellow_minor.png["Icône alerte jaune mineure"] *Attention* : Une ou plusieurs valeurs sont hors de la plage normale.
* image:icon_alert_red_critical.png["Icône alerte rouge critique"] *Attention* : Une ou plusieurs des valeurs sont significativement en dehors de la plage normale.
...
+
image:support_diagnostics_page.png["Page support Diagnostics"]

. Pour en savoir plus sur un diagnostic spécifique, sélectionnez cette ligne.
+
...
+
image:support_diagnostics_cpu_utilization.png["Exemple de valeurs d'état de diagnostic"]
...
+
image:grafana_dashboard_nodes.png["Nœuds de tableau de bord Grafana"]

. *Facultatif* : Pour afficher un graphique de l’expression Prometheus au fil du temps, sélectionnez *Afficher dans Prometheus*.
+
Un graphique Prometheus de l'expression utilisée dans le diagnostic s'affiche.

+
image:support_diagnostics_prometheus_png.png["Page support Diagnostics Prometheus"]
