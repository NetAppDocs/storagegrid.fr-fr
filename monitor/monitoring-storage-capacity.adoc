...
---
= Surveillez la capacité de stockage dans StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
image:dashboard_data_and_metadata_space_usage_breakdown.png["Répartition de l'utilisation de l'espace de données et de métadonnées"]
...
+
image:dashboard_storage_over_time.png["Graphique stockage dans le temps"]
...
+
image:nodes_deployment_storage_tab.png["Onglet stockage de déploiement des nœuds"]
...
image:calculating_watermarks.gif["Dessin conceptuel de l'espace utilisable total"]
...
+
image:nodes_page_storage_used_object_data.png["Nœuds stockage de page données d'objet utilisées"]
...
NOTE: Pour visualiser les graphiques de ces valeurs, cliquez sur les icônes de graphique image:icon_chart_new_for_11_5.png["Icône de graphique"] dans les colonnes disponibles.

+
image:nodes_page_storage_tables.png["Les tables de stockage d'objets sont stockées"]
...
image:forecast-metadata-usage.png["Prévision de l'utilisation des métadonnées par site"]
