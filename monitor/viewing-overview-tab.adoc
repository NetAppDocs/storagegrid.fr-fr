...
---
= Consultez l'onglet Vue d'ensemble StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
image:nodes_page_overview_tab.png["Onglet vue d'ensemble de la page noeuds"]
...
+
** *Inconnu* image:icon_alarm_blue_unknown.png["icône bleue du point d'interrogation"] : Pour une raison inconnue, le nœud n’est pas connecté à la grille, ou un ou plusieurs services sont indisponibles de façon inattendue. Par exemple, la connexion réseau entre les nœuds a été perdue, il y a une coupure de courant ou un service est indisponible. L’alerte *Unable to communicate with node* peut également être déclenchée. D’autres alertes peuvent également être actives. Cette situation requiert une attention immédiate.
+
...
** *Hors service administrativement* image:icon_alarm_gray_administratively_down.png["icône questionmark grise"] : Le nœud n’est pas connecté au réseau pour une raison prévue. Par exemple, le nœud ou les services sur le nœud ont été arrêtés correctement, le nœud est en cours de redémarrage ou le logiciel est en cours de mise à niveau. Une ou plusieurs alertes peuvent également être actives.
** *Connecté* image:icon_alert_green_checkmark.png["icône alerte verte coche"]: Le nœud est connecté au réseau.
...
* *Adresses IP* : Les adresses IP du nœud. Sélectionnez *Afficher les adresses IP supplémentaires* pour afficher les adresses IPv4 et IPv6 du nœud ainsi que les mappages d’interface.
...
image:nodes_page_alerts_table.png["Tableau des alertes de la page nœuds"]
