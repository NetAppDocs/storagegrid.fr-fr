...
summary: 'La fonctionnalité AutoSupport permet à votre système StorageGRID d"envoyer des messages périodiques et déclenchés par des événements concernant l"état et la santé au support technique afin de permettre une surveillance proactive et un dépannage. StorageGRID AutoSupport permet également l"utilisation de Digital Advisor pour des recommandations prédictives.' 
---
...
Par défaut, les fonctionnalités hebdomadaires et déclenchées par événement AutoSupport sont activées pour les systèmes StorageGRID logiciels lors de la première installation de StorageGRID. Cependant, vous devez <<autosupport-for-appliances,Configurer le matériel AutoSupport>> sur chaque appliance. Au besoin, vous pouvez modifier la configuration AutoSupport.


NOTE: Si vous souhaitez modifier la configuration de StorageGRID AutoSupport, effectuez vos modifications uniquement sur le nœud d'administration principal.

.Avant de commencer
* Vous êtes connecté au Grid Manager sur le nœud d'administration principal à l'aide d'link:../admin/web-browser-requirements.html["navigateur web pris en charge"].
* Vous avez le link:../admin/admin-group-permissions.html["Autorisation d'accès racine"].
* Si vous utilisez HTTPS pour envoyer des packages AutoSupport, vous avez fourni un accès Internet sortant au nœud d'administration principal, soit directement, soit link:../admin/configuring-admin-proxy-settings.html["utilisation d'un serveur proxy"] (les connexions entrantes ne sont pas requises).
* Si HTTP est sélectionné sur la page AutoSupport de StorageGRID, vous devez link:../admin/configuring-admin-proxy-settings.html["configurez un serveur proxy"] transférer les packages AutoSupport en HTTPS. Les serveurs AutoSupport de NetApp rejetteront les packages envoyés via HTTP.
* Si vous utilisez SMTP comme protocole pour les paquets AutoSupport, vous avez link:../monitor/configure-email-server.html["configuré un serveur de messagerie"].
...
== [[configure-autosupport]]Configurer AutoSupport

Pour modifier la configuration d'installation par défaut de AutoSupport, ou pour configurer le matériel AutoSupport sur chaque appliance StorageGRID :
...
** *HTTP* : si vous sélectionnez HTTP, vous devez link:../admin/configuring-admin-proxy-settings.html["configurer un serveur proxy"] pour transférer les packages AutoSupport en HTTPS. Les serveurs AutoSupport de NetApp rejettent les packages envoyés via HTTP. Ce protocole utilise le port 80.
** *SMTP* : Utilisez cette option si vous souhaitez que les paquets AutoSupport soient envoyés par e-mail. Vous devez d’abord link:../monitor/configure-email-server.html["configurer un serveur de messagerie"].


+
Le protocole que vous avez défini est utilisé pour l'envoi de tous les paquets AutoSupport sauf AutoSupport on Demand, qui nécessite le protocole HTTPS.
...
. Si vous avez sélectionné *SMTP*, utilisez le champ *Expéditeur* pour saisir l'adresse du serveur de messagerie que vous avez configuré précédemment.
...
Pour vérifier ou modifier le protocole utilisé pour envoyer des paquets AutoSupport, consultez les instructions pour <<configure-autosupport,configurer AutoSupport>>.
...
AutoSupport packages des appliances de la série E peuvent être inclus dans StorageGRID AutoSupport si vous configurez la livraison AutoSupport par proxy dans link:sending-eseries-autosupport-messages-through-storagegrid.html["SANtricity System Manager"].
...
* link:../network/external-communications.html["Ports utilisés pour les communications externes"]
