...
summary: 'Vous devez effectuer un ensemble spécifique de tâches pour effectuer une restauration suite à une défaillance d"un nœud d"administration principal. Le nœud d"administration principal héberge le service de nœud de gestion de la configuration (CMN) pour la grille.' 
---
= Récupérez le nœud d'administration principal StorageGRID en utilisant le même nœud d'administration principal.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
CAUTION: Vous devez réparer ou remplacer rapidement un nœud d'administration principal défaillant, faute de quoi vous risquez de ne pas pouvoir effectuer certaines opérations de maintenance.
