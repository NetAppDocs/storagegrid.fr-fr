...
summary: 'Vous devrez peut-être désaffecter un nœud qui n"est pas actuellement connecté à la grille (dont l"état de santé est inconnu ou désactivé d"un point de vue administratif).' 
---
...
Vous pouvez identifier les nœuds déconnectés en recherchant l’icône bleue Inconnu image:icon_alarm_blue_unknown.png["Icône inconnue"] ou l’icône grise Administrativement hors service image:icon_alarm_gray_administratively_down.png["Icône administrative vers le bas"] dans la colonne *Santé*.
...
. Examinez la liste des nœuds, puis sélectionnez *OK*.
+
...
. Dès que le nouveau package de récupération est disponible, sélectionnez le lien ou sélectionnez *Maintenance* > *System* > *Recovery package* pour accéder à la page du package de récupération. Téléchargez ensuite le  `.zip` fichier.
+
...
--
Pour surveiller la réparation des données à code d'effacement et relancer toute requête qui aurait pu échouer, sélectionnez *Support* > *Outils* > *Métriques* pour afficher le temps estimé jusqu'à l'achèvement et le pourcentage d'achèvement de la tâche en cours. Ensuite, sélectionnez *EC Overview* dans la section Grafana. Consultez les tableaux de bord *Grid EC Job Estimated Time to Completion* et *Grid EC Job Percentage Completed*.
