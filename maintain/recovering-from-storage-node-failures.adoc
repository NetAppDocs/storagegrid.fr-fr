...
---
= Récupérer après des pannes de nœuds de stockage StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Contactez l'assistance technique.
 a| 
En cas de défaillance de plusieurs nœuds de stockage, l'assistance technique doit intervenir pour la récupération afin d'éviter les incohérences de la base de données Cassandra susceptibles d'entraîner une perte de données. Une procédure de restauration du site peut s'avérer nécessaire.

Le support technique peut déterminer à quel moment il est sûr de commencer la récupération d'un deuxième nœud de stockage.

*Remarque* : Si plusieurs nœuds de stockage contenant le service ADC tombent en panne sur un site, toutes les demandes de service de plateforme en attente pour ce site seront perdues.
...
Un ou plusieurs volumes de stockage sont en panne, mais le lecteur système est intact
 a| 
link:reviewing-warnings-about-storage-volume-recovery.html["Restaurez le disque d'après la panne du volume de stockage là où le disque du système est intact"]
 a| 
