...
summary: Si la procédure automatique ne parvient pas à remonter les volumes de stockage préservés et à reformater les volumes de stockage défaillants, vous devez exécuter manuellement deux scripts pour effectuer ces opérations. Le premier script remonte les volumes correctement formatés en tant que volumes de stockage StorageGRID. Le second script reformate les volumes non montés, reconstruit la base de données Cassandra si nécessaire et démarre les services. 
---
= Remonter et reformater les volumes de stockage de l'appliance StorageGRID (étapes manuelles)
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Si la procédure automatique ne parvient pas à remonter les volumes de stockage préservés et à reformater les volumes de stockage défaillants, vous devez exécuter manuellement deux scripts pour effectuer ces opérations. Le premier script remonte les volumes correctement formatés en tant que volumes de stockage StorageGRID. Le second script reformate les volumes non montés, reconstruit la base de données Cassandra si nécessaire et démarre les services.

Lors de la récupération d'un nœud de stockage, si le message « En attente d'instructions manuelles » s'affiche dans le Gestionnaire de grille, suivez ces étapes pour remonter et reformater les volumes de stockage. Si ce message ne s'affiche pas, les opérations de remontage et de reformatage se sont terminées automatiquement, et vous pouvez ignorer _cette_ procédure et link:../maintain/restoring-object-data-to-storage-volume-for-appliance.html["restaurer les données de l'objet"].
...
+
image:recovering_cassandra.png["Capture d'écran montrant la progression de la récupération dans Grid Management interface"]
...
+
Le script vous demande si vous souhaitez utiliser le processus de restauration de volume de Grid Manager pour restaurer les données des objets.

+
** Dans la plupart des cas, vous devriez link:../maintain/restoring-object-data-to-storage-volume-for-appliance.html["Restaurez les données d'objet à l'aide de Grid Manager"]. Répondez `y` pour utiliser le Gestionnaire de grille.
** Dans de rares cas, par exemple si le nœud de remplacement dispose de moins de volumes disponibles pour le stockage d'objets que le nœud d'origine, contactez le support technique pour restaurer manuellement les données d'objets. Si l'un de ces cas s'applique, répondez `n` et appuyez sur n'importe quelle touche pour revenir à la ligne de commandes.
