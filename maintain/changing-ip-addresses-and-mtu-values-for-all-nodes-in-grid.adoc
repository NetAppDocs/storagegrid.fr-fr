...
---
= Modifier les adresses IP de tous les nœuds dans une grille StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: Si l'authentification unique (SSO) est activée pour votre système StorageGRID et que des approbations de parties de confiance ont été configurées à l'aide d'adresses IP de nœuds d'administration (au lieu de noms de domaine complets, comme recommandé), soyez prêt à mettre à jour ou à reconfigurer ces approbations de parties de confiance dans Active Directory Federation Services (AD FS) immédiatement après avoir modifié les adresses IP. Reportez-vous à la configuration de l'authentification unique à l'aide de link:../admin/learn-about-setting-up-sso-oidc.html["OIDC"] ou link:../admin/learn-about-setting-up-sso-saml.html["SAML"].
