...
---
= Sélectionnez un site à mettre hors service dans StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
image:decommission_site_step_select_site.png["Désaffectez le site étape 1 sélectionnez site"]
...
| Motif de mise hors service possible | Description | Étape suivante 
 ...
 a| 
Coche verte (image:icon_alert_green_checkmark.png["Icône alerte verte coche"])
 a| 
...
Non. Tous les nœuds de ce site sont déconnectés. Contactez votre ingénieur commercial NetApp.
 a| 
Vous ne pouvez pas effectuer la mise hors service d'un site connecté à moins que chaque nœud du site ne soit connecté (image:icon_alert_green_checkmark.png["Icône alerte verte coche"]).
 a| 
...
|===
+
L'exemple illustre un système StorageGRID avec trois sites. La coche verte (image:icon_alert_green_checkmark.png["Icône alerte verte coche"]) pour les sites de Raleigh et Sunnyvale indique que vous pouvez mettre ces sites hors service. Cependant, vous ne pouvez pas mettre le site de Vancouver hors service car il contient le nœud d'administration principal.
