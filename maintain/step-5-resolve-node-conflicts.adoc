...
* Tous les nœuds de votre système StorageGRID doivent être connectés (image:icon_alert_green_checkmark.png["Icône alerte verte coche"]).
+
...
.Étapes
. Consultez la section des nœuds déconnectés de l’étape 5 (Résoudre les conflits de nœuds) pour déterminer si des nœuds de votre système StorageGRID ont un état de connexion Inconnu (image:icon_alarm_blue_unknown.png["Icône alarme bleue inconnue"]) ou Administrativement hors service (image:icon_alarm_gray_administratively_down.png["Icône alarme grise - Arrêt administratif"]).
+
image:decommission_site_step_5_disconnected_nodes.png["Désaffectez le site étape 5 résoudre les conflits de nœud"]
...
+
image:decommission_site_step_5_ha_groups.png["Désaffectation du site étape 5 groupes haute disponibilité"]
...
+
Si tous les nœuds sont connectés et qu'aucun nœud du site sélectionné n'est utilisé dans un groupe HA, le champ *Provisioning passphrase* est activé.
...
+
image:decommission_site_step_5_provision_passphrase.png["Désaffectez la phrase de passe de l'étape 5 du site"]
...
+
image:decommission_site_step_5_warning.png["Mise hors service du site - étape 5 Avertissement"]
...
+
image:decommission_site_step_5_grid_configuration.png["Désaffectez la configuration de la grille étape 5 du site"]
