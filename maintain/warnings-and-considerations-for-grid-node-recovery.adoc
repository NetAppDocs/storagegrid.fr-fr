...
---
= Avertissements et considérations pour la récupération des nœuds StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Si vous récupérez un nœud de grille autre que le nœud d'administration principal, une connectivité doit exister entre le nœud de grille en cours de récupération et le nœud d'administration gérant la procédure de récupération.
...
Si un serveur hébergeant plusieurs nœuds de grille tombe en panne, vous pouvez restaurer les nœuds dans n'importe quel ordre. Cependant, si le serveur défaillant héberge le nœud d'administration principal et qu'aucun nœud d'administration non principal n'existe, vous devez restaurer le nœud d'administration principal en premier.
