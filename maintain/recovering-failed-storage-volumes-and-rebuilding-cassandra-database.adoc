...
---
= Récupérez les volumes de stockage défaillants et reconstruisez la base de données Cassandra dans StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.Et ensuite ?
Une fois les volumes de stockage reformatés et remontés et les opérations Cassandra nécessaires terminées, vous pouvez link:../maintain/restoring-object-data-to-storage-volume-where-system-drive-is-intact.html["Restaurez les données d'objet à l'aide de Grid Manager"].
