...
* Vous connaissez l'adresse IP de l'un des nœuds d'administration suivants :
+
** Le nœud d'administration principal de la grille StorageGRID
** Un nœud d'administration non principal, si disponible
...
* Vous indiquez l'adresse IP du nœud d'administration principal. Si le nœud d'administration principal est indisponible, vous indiquez l'adresse IP du nœud d'administration non principal, si disponible. Vous confirmez également le nom d'hôte (nom système) du nœud.
...
. Dans le champ *Nom du nœud*, saisissez le même nom d’hôte (nom du système) que celui utilisé pour le nœud que vous récupérez, puis sélectionnez *Enregistrer*.
. Dans la section Installation, vérifiez que l’état actuel est « Prêt à démarrer l’installation de `_node name_` dans la grille avec le nœud d’administration principal `_admin_ip_` » et que le bouton *Démarrer l’installation* est activé.
+
...
NOTE: Si vous devez accéder manuellement à la page d'installation de Monitor, sélectionnez *Monitor installation* dans la barre de menu. Voir https://docs.netapp.com/us-en/storagegrid-appliances/installconfig/monitoring-appliance-installation.html["Surveiller l'installation de l'appareil"^].
