...
---
= Restaurez les nœuds StorageGRID sur l'hôte
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Si vous récupérez un nœud à partir d'un nœud d'administration non principal, définissez ADMIN_IP sur l'adresse IP du réseau Grid de ce nœud d'administration non principal, afin que le nœud récupéré s'enregistre auprès du nœud d'administration non principal plutôt qu'auprès du nœud d'administration principal. Pour consulter les règles relatives aux clés du réseau d'administration, reportez-vous à link:../swnodes/creating-node-configuration-files.html["Créer des fichiers de configuration de nœud StorageGRID pour les déploiements Linux"].
