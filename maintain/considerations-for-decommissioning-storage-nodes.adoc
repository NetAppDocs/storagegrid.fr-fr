...
---
= Considérations relatives à la mise hors service des nœuds de stockage dans StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Le système doit, à tout moment, comporter suffisamment de nœuds de stockage pour répondre aux exigences opérationnelles, y compris le link:understanding-adc-service-quorum.html["Quorum ADC"] et l’link:reviewing-ilm-policy-and-storage-configuration.html["Politique ILM"] actif. Si une mise hors service devait faire passer le nombre d’ADC d’un site en dessous du quorum, StorageGRID déplace automatiquement le service ADC vers des nœuds de stockage non-ADC disponibles sur le même site, maintenant ainsi le quorum ADC. Si le service ADC ne peut pas être déplacé automatiquement, vous devrez peut-être link:../expand/adding-grid-nodes-to-existing-site-or-adding-new-site.html["ajouter un nouveau nœud de stockage dans une opération d'expansion"] avant de pouvoir mettre hors service un nœud de stockage existant.
+
