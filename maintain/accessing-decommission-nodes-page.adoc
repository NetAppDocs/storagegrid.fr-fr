...
---
= Accédez à la page Décommissionnement des nœuds dans StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
Dans cet exemple, une coche verte dans la colonne « Désactivation possible » indique que vous pouvez désactiver le nœud API Gateway et le nœud de stockage de données uniquement. Les autres nœuds ne peuvent pas être désactivés pour diverses raisons. Par exemple, le nœud d'administration principal ne peut pas être désactivé car la désactivation de ce type de nœud n'est pas prise en charge. Le nœud de stockage de métadonnées uniquement ne peut pas être désactivé car le site ne répondrait plus aux link:../maintain/understanding-adc-service-quorum.html["Quorum ADC"].

+
image:decommission_nodes_page.png["Page Désaffectez les nœuds"]
...
** image:icon_alarm_gray_administratively_down.png["icône questionmark grise"] (gris) : Administrativement désactivé
** image:icon_alarm_blue_unknown.png["icône bleue du point d'interrogation"] (bleu) : Inconnu
...
Modifiez le groupe haute disponibilité pour supprimer l'interface du nœud ou supprimer l'ensemble du groupe haute disponibilité. Voir link:../admin/configure-high-availability-group.html["Configurez les groupes haute disponibilité"].
 ...
 a| 
Non, le site _x_ nécessite au moins 3 nœuds de stockage avec services ADC ou des nœuds non-ADC disponibles pour y transférer les services ADC. La mise hors service d'un nœud ADC n'est pas disponible si des nœuds sont hors ligne.
 a| 
*Nœuds de stockage uniquement* (nœuds de stockage combinés ou de métadonnées uniquement). Vous ne pouvez pas mettre hors service un nœud de stockage si le nombre de nœuds restants sur le site est insuffisant pour respecter les exigences de quorum de l'ADC et si le service ADC ne peut pas être déplacé automatiquement. Le déplacement automatique nécessite la présence de plus de deux nœuds de stockage ADC en ligne sur la grille et de nœuds de stockage non-ADC disponibles sur le même site.
 a| 
Assurez-vous que les conditions pour le déplacement automatique du service ADC sont remplies (voir link:../maintain/understanding-adc-service-quorum.html["Quorum ADC"]), ou link:../maintain/move-adc-service.html["déplacer manuellement le service ADC"] vers un autre nœud de stockage sur le même site. Vous pouvez également effectuer une extension en ajoutant un nouveau nœud de stockage au site et en spécifiant qu'il doit avoir un service ADC. Voir link:../expand/adding-grid-nodes-to-existing-site-or-adding-new-site.html["Résumé du workflow d'ajout de nœuds de grille à StorageGRID"].
