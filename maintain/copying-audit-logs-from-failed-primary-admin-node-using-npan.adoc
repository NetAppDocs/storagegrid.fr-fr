...
---
= Copiez les journaux d'audit StorageGRID à partir d'un nœud d'administration principal défaillant
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.. Saisissez la commande suivante pour passer au compte utilisateur root : `su -`
.. Saisissez le mot de passe indiqué dans le `Passwords.txt` fichier.
+
...
+
Lorsque vous y êtes invité, saisissez le mot de passe du compte utilisateur admin.
...
. Déconnectez-vous en tant qu'utilisateur root : `exit`.
