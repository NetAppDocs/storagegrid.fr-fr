...
summary: 'Vous pouvez ajouter, supprimer et mettre à jour des serveurs DNS afin que vous puissiez utiliser des noms d"hôtes de domaine complet (FQDN) plutôt que des adresses IP.' 
---
= Configurer les serveurs DNS dans StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.Étapes
. Sélectionnez *Maintenance* > *Réseau* > *Serveurs DNS*.
. Sélectionnez l'icône de suppression image:icon-x-to-remove.png["icône supprimer"] à côté de l'adresse IP.
. Sélectionnez *Enregistrer*.
