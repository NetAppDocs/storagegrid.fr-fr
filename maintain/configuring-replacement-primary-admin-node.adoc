...
* Vous disposez de la dernière sauvegarde du fichier de package de récupération (`sgws-recovery-package-_id-revision_.zip`).
...
.. Recherchez le fichier de package de récupération le plus récent pour votre système StorageGRID, puis cliquez sur *Ouvrir*.
...
NOTE: Pour configurer une confiance de partie utilisatrice, voir link:../admin/configure-sso.html["Configurer l'authentification unique"]. Pour accéder au certificat de serveur par défaut, connectez-vous au shell de commande du noeud d'administration. Accédez au `/var/local/mgmt-api` répertoire et sélectionnez le `server.crt` fichier.
