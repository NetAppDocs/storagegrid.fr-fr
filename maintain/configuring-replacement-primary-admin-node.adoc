...
---
= Configurez un nœud d'administration principal de remplacement dans StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
. Sélectionnez *Récupérer un nœud d'administration principal défaillant*.
+
image:install_or_recover_primary_admin_node.png["Installez ou récupérez le nœud d'administration principal"]

. Téléchargez la sauvegarde la plus récente du package de récupération :
+
.. Sélectionnez *Parcourir*.
.. Localisez le fichier de package de récupération le plus récent pour votre système StorageGRID, puis sélectionnez *Ouvrir*.
...
NOTE: Pour configurer une approbation de partie de confiance, reportez-vous à la configuration de l’authentification unique (SSO) à l’aide de link:../admin/learn-about-setting-up-sso-oidc.html["OIDC"] ou link:../admin/learn-about-setting-up-sso-saml.html["SAML"]. Pour accéder au certificat serveur par défaut, connectez-vous au shell de commande du nœud d’administration. Accédez au répertoire  `/var/local/mgmt-api`, puis sélectionnez le fichier  `server.crt`.
