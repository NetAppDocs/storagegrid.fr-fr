...
---
= Considérations relatives à la mise hors service des sites dans StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: Si un site contient un mélange de nœuds connectés (image:icon_alert_green_checkmark.png["Icône alerte verte coche"]) et de nœuds déconnectés (image:icon_alarm_gray_administratively_down.png["Icône alarme grise - Arrêt administratif"] ou image:icon_alarm_blue_unknown.png["Icône alarme bleue inconnue"]), vous devez remettre en ligne tous les nœuds hors ligne.
...
* Vous ne pouvez pas mettre hors service un site s'il contient un mélange de nœuds connectés (image:icon_alert_green_checkmark.png["Icône alerte verte coche"]) et déconnectés (image:icon_alarm_blue_unknown.png["Icône alarme bleue inconnue"] ou image:icon_alarm_gray_administratively_down.png["Icône alarme grise - Arrêt administratif"]).
* Vous ne pouvez pas mettre hors service un site si un nœud d'un autre site est déconnecté (image:icon_alarm_blue_unknown.png["Icône alarme bleue inconnue"] ou image:icon_alarm_gray_administratively_down.png["Icône alarme grise - Arrêt administratif"]).
...
* Tous les nœuds de votre système StorageGRID doivent avoir un état de connexion *Connecté* (image:icon_alert_green_checkmark.png["Icône alerte verte coche"]); cependant, les nœuds peuvent avoir des alertes actives.
+
...
+
** *Inconnu* (image:icon_alarm_blue_unknown.png["Icône alarme bleue inconnue"]) : Pour une raison inconnue, un nœud est déconnecté ou les services sur le nœud sont indisponibles de manière inattendue. Par exemple, un service sur le nœud peut être arrêté, ou le nœud peut avoir perdu sa connexion réseau en raison d'une panne de courant ou d'une interruption inattendue.
** *Hors service administrativement* (image:icon_alarm_gray_administratively_down.png["Icône alarme grise - Arrêt administratif"]) : Le nœud n’est pas connecté à la grille pour une raison prévue. Par exemple, le nœud ou les services sur le nœud ont été arrêtés correctement.


* Tous les nœuds de tous les autres sites doivent avoir un état de connexion *Connecté* (image:icon_alert_green_checkmark.png["Icône alerte verte coche"]); cependant, ces autres nœuds peuvent avoir des alertes actives.
