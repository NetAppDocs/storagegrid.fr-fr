...
summary: 'Vous pouvez modifier la configuration réseau d"un ou plusieurs nœuds à l"aide de l"outil Modifier l"adresse IP. Vous pouvez modifier la configuration du réseau Grid, ou ajouter, modifier ou supprimer les réseaux Admin ou Client. Vous pouvez également modifier les paramètres IP LAN d"un nœud BMC.' 
---
= Modifier la configuration réseau du nœud dans StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Vous pouvez modifier la configuration réseau d'un ou plusieurs nœuds à l'aide de l'outil Modifier l'adresse IP. Vous pouvez modifier la configuration du réseau Grid, ou ajouter, modifier ou supprimer les réseaux Admin ou Client. Vous pouvez également modifier les paramètres IP LAN d'un nœud BMC.
...
NOTE: Si l’authentification unique (SSO) est activée pour votre système StorageGRID et que vous modifiez l’adresse IP d’un nœud d’administration, sachez que toute approbation de partie de confiance configurée à l’aide de l’adresse IP du nœud d’administration (au lieu de son nom de domaine complet, comme recommandé) deviendra invalide. Vous ne pourrez plus vous connecter au nœud. Immédiatement après la modification de l’adresse IP, vous devez mettre à jour ou reconfigurer l’approbation de partie de confiance du nœud dans Active Directory Federation Services (AD FS) avec la nouvelle adresse IP. Consultez les instructions de configuration de l’authentification unique (SSO) à l’aide de link:../admin/learn-about-setting-up-sso-oidc.html["OIDC"] ou link:../admin/learn-about-setting-up-sso-saml.html["SAML"].
...
+
image:change_ip_tool_main_menu.png["Capture d'écran affichant l'écran d'accueil de l'outil modifier IP"]
...
+
image:change_ip_tool_edit_ip_mask_sample_output.png["capture d'écran décrite par le texte environnant"]
...
+
image:change_ip_tool_validate_sample_error_messages.gif["capture d'écran décrite par le texte environnant"]

+
Dans cet exemple, la validation a réussi.

+
image:change_ip_tool_validate_sample_passed_messages.gif["capture d'écran décrite par le texte environnant"]
...
== Modifier les paramètres IP LAN du BMC du nœud de l'appliance

Vous pouvez modifier les paramètres IP LAN d'un nœud BMC.

. Dans Grid Manager, accédez à *Nœuds* > *_grid node_* > *Paramètres BMC*.
. Dans la section des paramètres IP du réseau local, sélectionnez un type d’attribution d’adresse IP *Statique* ou *DHCP*.
. Saisissez l'adresse MAC, l'adresse IPv4 (CIDR) et la passerelle par défaut du BMC.
. Sélectionnez *Enregistrer*.


Vous pouvez également modifier ou consulter les paramètres IP LAN du BMC à l'aide de l' `PUT /bmc-configuration/root-password` ou de l' `GET /bmc-configuration/root-password`API.
