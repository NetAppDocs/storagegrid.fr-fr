...
---
= Créez un pool de stockage dans StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: Lors de l'installation de StorageGRID 12.0 ou version ultérieure sur une nouvelle grille, des pools de stockage sont automatiquement créés pour chaque site. En revanche, si vous avez initialement installé StorageGRID 11.6 ou une version antérieure, les pools de stockage ne sont pas créés automatiquement pour chaque site.
...
+
Pour supprimer une entrée, sélectionnez l'icône de suppression image:icon_x_to_remove.png["icône supprimer"].
