...
Si un client supprime une version d'objet avec un ID de version link:../tenant/manage-branch-buckets.html["seau de dérivation"]d'un , StorageGRID masque l'existence de la version d'objet mais ne crée pas de marqueur de suppression dans le compartiment de branche. Si un client supprime un objet sans ID de version d'un compartiment de succursale, StorageGRID crée un marqueur de suppression dans ce compartiment en fonction du comportement S3 standard.
