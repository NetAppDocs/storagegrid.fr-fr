...
---
= Protection contre les pertes de site grâce aux pools de stockage dans StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
image:ilm_replication_at_2_sites.png["Réplication d'une copie sur deux sites"]

image:ilm_replication_make_2_copies_2_pools_2_sites.png["Faites 1 copie sur chacun des 2 sites - pool de stockage spécifique au site"]
...
image:ilm_erasure_coding_site_loss_protection_4+2.png["Pool de stockage EC 1 9 nœuds de stockage 3 sites"]
...
image:ec_three_sites_4_plus_2_site_loss_example_template.png["Stockage d'objets par code d'effacement utilisant un schéma de code d'effacement 4+2 sur 3 sites"]

En cas de perte d'un site, les données peuvent toujours être restaurées :

image:ec_three_sites_4_plus_2_site_loss_example.png["Code d'effacement 4+2 EC schéma 1 site perdu"]
