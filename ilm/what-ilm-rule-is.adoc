...
summary: 'Pour gérer les objets, vous créez un ensemble de règles de gestion du cycle de vie des informations (ILM) et vous les organisez en une règle ILM.' 
---
= Découvrez les règles ILM dans StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
image:ilm_rule_multiple_placements_in_single_time_period.png["Écran de placement ILM de Grid Manager"]

La première consigne de placement image:icon_number_1.png["Icône numéro 1"] comporte deux lignes pour la première année :
...
La deuxième instruction de placement image:icon_number_2.png["Icône numéro 1"] crée deux copies après un an et les conserve indéfiniment.
