...
summary: 'L"étape Définir les emplacements de l"assistant Créer une règle ILM vous permet de définir les instructions d"emplacement qui déterminent la durée de stockage des objets, le type de copies (répliquées ou code d"effacement), l"emplacement de stockage et le nombre de copies.' 
---
= Définir les instructions de placement pour une règle StorageGRID ILM
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
image:ilm_create_ilm_rule_wizard_2.png["Assistant de création de règles ILM étape 2"]
...
+
image:ilm_rule_retention_diagram.png["Schéma de conservation des règles ILM"]
