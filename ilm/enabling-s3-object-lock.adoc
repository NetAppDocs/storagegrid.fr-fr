...
---
= Activer le verrouillage d'objets S3 dans StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Vous avez confirmé que la règle par défaut de la stratégie ILM active est conforme. Consultez link:creating-default-ilm-rule.html["Créez une règle ILM par défaut"] pour plus de détails.
...
Après avoir activé le verrouillage d'objet S3, un administrateur de grille peut également autoriser les utilisateurs du locataire à activer le verrouillage d'objet S3 sur un compartiment existant. Après qu'un administrateur de grille a activé ce paramètre, il peut le désactiver. Pour plus d'informations, consultez link:requirements-for-s3-object-lock.html#s3-object-lock-existing-buckets["Exigences de verrouillage d'objet S3"].
...
. Sélectionnez *Activer le verrouillage d'objet S3* pour activer le verrouillage d'objet S3 pour l'ensemble de votre système StorageGRID.
. Vous pouvez également sélectionner *Autoriser l’activation du verrouillage d’objets S3 sur les compartiments existants*.
+

CAUTION: L'activation du verrouillage d'objet S3 sur un compartiment existant peut entraîner une migration des données d'un pool de stockage cloud vers StorageGRID, selon vos règles ILM. Vous ne pouvez pas arrêter cette migration une fois qu'elle a commencé. Pour plus d'informations, consultez link:requirements-for-s3-object-lock.html#s3-object-lock-existing-buckets["Exigences de verrouillage d'objet S3"].
...
** Si la règle par défaut n'est pas conforme, une erreur s'affiche. Vous devez créer et activer une nouvelle stratégie ILM qui inclut une règle conforme comme règle par défaut. Sélectionnez *OK*. Ensuite, créez une nouvelle stratégie, simulez-la et activez-la. Consultez link:creating-ilm-policy.html["Création de la règle ILM"] pour obtenir des instructions.
