...
---
= Créer des règles ILM dans StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: Si link:enabling-s3-object-lock.html["Le paramètre de verrouillage d'objet S3 global a été activé"], vous devez vous assurer que la stratégie ILM est conforme aux exigences des compartiments pour lesquels le verrouillage d'objets S3 est activé. Dans cette section, suivez les instructions qui mentionnent l'activation du verrouillage d'objets S3.
...
--
** Le link:enabling-s3-object-lock.html["Le paramètre de verrouillage d'objet S3 global est déjà activé"] pour le système StorageGRID.
...
.Résultats
Toute modification apportée aux règles de la stratégie sera prise en compte dans les résultats de la simulation et affichera la nouvelle correspondance ainsi que la correspondance précédente. La fenêtre de simulation de stratégie conserve les objets testés jusqu'à ce que vous sélectionniez *Tout effacer* ou l'icône de suppression image:icon_x_to_remove.png["Supprimer l'icône"] pour chaque objet dans la liste des résultats de la simulation.
