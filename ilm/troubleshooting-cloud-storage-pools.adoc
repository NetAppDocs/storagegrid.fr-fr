...
---
= Dépanner les pools de stockage cloud dans StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Vous pourriez rencontrer cette erreur dans l'un des cas suivants :

* Lorsque vous activez le verrouillage d'objets S3 avec la rétention par défaut pour votre compartiment Amazon S3 après avoir commencé à utiliser ce compartiment comme pool de stockage cloud. Cette erreur se produit lorsque l'opération PUT ne comporte pas d'en-tête HTTP avec une valeur de somme de contrôle de la charge utile, telle que  `Content-MD5`. Cette valeur d'en-tête est requise par AWS pour les opérations PUT dans des compartiments avec le verrouillage d'objets S3 activé.
+
Pour corriger ce problème, suivez les étapes dans link:../ilm/editing-cloud-storage-pool.html["Modifiez un pool de stockage cloud"] sans apporter de modifications. Cette action déclenche la validation de la configuration du Cloud Storage Pool qui détecte et met à jour automatiquement l'indicateur S3 Object Lock sur la configuration du point de terminaison du Cloud Storage Pool.

* Lorsque vous activez le verrouillage d'objet S3 avec la rétention par défaut pour votre compartiment StorageGRID S3 après avoir commencé à utiliser ce compartiment pour un pool de stockage cloud.
+
Pour corriger ce problème :

+
** Si vous utilisez HTTP : les pools de stockage cloud avec S3 Object Lock activé ne sont pas pris en charge. Modifiez la connexion du point de terminaison pour utiliser HTTPS à la place.
** Si vous utilisez HTTPS : suivez les étapes dans link:../ilm/editing-cloud-storage-pool.html["Modifiez un pool de stockage cloud"] sans apporter de modifications. Cette action déclenche la validation de la configuration du Cloud Storage Pool qui détecte et met à jour automatiquement l’indicateur S3 Object Lock sur la configuration du point de terminaison du Cloud Storage Pool.




En cas de problème supplémentaire, contactez le support technique.
