...
* Vous avez link:../admin/configure-sso.html#enter-sandbox-mode-adfs["entrée en mode sandbox"] dans le Gestionnaire de grille.
* Vous connaissez le nom de domaine complet (ou l'adresse IP) et l'identifiant de partie comptant pour chaque nœud d'administration de votre système. Vous trouverez ces valeurs dans le tableau détaillé noeuds d'administration de la page configurer SSO StorageGRID.
+
...
. Lorsque vous avez terminé, revenez à StorageGRID et link:../admin/configure-sso.html#test-sso["tester toutes les fiducies de la partie qui repose"]vérifiez qu'ils sont correctement configurés.
