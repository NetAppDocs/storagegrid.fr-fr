...
---
= Créer des approbations de parties de confiance dans AD FS pour SAML dans StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Vous devez utiliser les services de fédération Active Directory (AD FS) pour créer une approbation de partie de confiance pour chaque nœud d'administration pour SAML dans votre système. Vous pouvez créer des approbations de partie de confiance à l'aide de commandes PowerShell, en important les métadonnées SAML depuis StorageGRID ou en saisissant les données manuellement.

.Avant de commencer
* Vous avez configuré l'authentification unique pour StorageGRID et sélectionné *AD FS* comme type SSO.
* Vous avez link:../admin/configure-sso-saml.html["entré en mode bac à sable"] dans Grid Manager.
...
. Une fois que vous avez terminé, retournez sur StorageGRID et link:../admin/configure-sso-saml.html#test-sso["tester toutes les fiducies des parties utilisatrices"] pour confirmer qu'ils sont correctement configurés.
...
. Répétez ces étapes pour configurer une confiance de tiers pour tous les nœuds d'administration de votre système StorageGRID.
. Une fois que vous avez terminé, retournez sur StorageGRID link:../admin/configure-sso-saml.html#test-sso["tester toutes les fiducies des parties utilisatrices"] pour confirmer qu'ils sont correctement configurés.
