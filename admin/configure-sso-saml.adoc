...
[role="lead"]
Suivez l'assistant de configuration SSO pour SAML et passez en mode sandbox pour configurer et tester l'authentification unique (SSO) avant de l'activer pour tous les utilisateurs StorageGRID. Après avoir activé SSO, vous pouvez revenir en mode sandbox si nécessaire pour modifier ou tester à nouveau la configuration.
...
Pour permettre à StorageGRID (le fournisseur de services) et au fournisseur d'identité SSO de communiquer de manière sécurisée au sujet des demandes d'authentification des utilisateurs, vous effectuez les tâches suivantes :
...
. Sous *Protocole*, sélectionnez *SAML*.
...
== [[enter-sandbox-mode]]Étape 2 : Fournissez les relations d’approbation de parties de confiance, les applications d'entreprise ou les connexions SP

Remplissez les champs de l'étape 2 de l'assistant en fonction du type de SSO que vous avez sélectionné.

[role="tabbed-block"]
====
.Active Directory
--
Remplissez les champs de l’étape *Fournir l’identifiant de la partie de confiance*.
...
.ID d'entrée
--
Remplissez les champs de l’étape *Configurer les applications d’entreprise*.
...
. Suivez les étapes décrites dans link:../admin/creating-enterprise-application-entra-id-saml.html["Créer des applications d'entreprise dans Entra ID"] pour créer une application d'entreprise pour chaque Admin Node répertorié dans le tableau.
...
.PingFederate
--
Remplissez les champs à l’étape *Fournir l’ID de connexion SP*.
...
== Étape 3 : Configurez les relations d’approbation des parties de confiance, les applications d'entreprise ou les connexions SP
...
+
Pour obtenir des instructions, rendez-vous sur link:../admin/creating-relying-party-trusts-in-ad-fs-saml.html["Créer des fiducies de tiers de confiance dans AD FS"].
...
. Suivez les étapes décrites link:../admin/creating-enterprise-application-entra-id-saml.html["Créer des applications d'entreprise dans Entra ID"] pour télécharger le fichier de métadonnées SAML pour chaque nœud d'administration dans son application d'entreprise Entra ID correspondante.
...
== [[test-sso]]Étape 4 : Tester la configuration
...
== [[enable-sso-saml]]Étape 5 : Activer l’authentification unique
...
.Et ensuite ?
Après avoir configuré l'authentification unique (SSO) à l'aide de SAML, configurez le fournisseur d'identité pour les Admin Nodes.

[cols="1a,2a"]
|===
| Si votre fournisseur d'identité est... | Vous devez... 


 a| 
Active Directory
 a| 
link:creating-relying-party-trusts-in-ad-fs-saml.html["Créez un groupe d'applications pour chaque nœud d'administration"]
...
ID d'entrée
 a| 
link:creating-enterprise-application-entra-id-saml.html["Créez une application pour chaque nœud d'administration"]
...
PingFederate
 a| 
link:creating-sp-connection-ping-saml.html["Créez une connexion de fournisseur de services pour chaque nœud d'administration"]
