...
NOTE: Pendant la modification de la connexion, les demandes de synchronisation de compte ou de réplication inter-grilles continueront d'utiliser les paramètres de connexion existants. Vous devez modifier une connexion à la fois et téléverser un fichier de vérification sur la grille distante pour chaque connexion. Les modifications sont appliquées après avoir été téléversées sur les grilles distantes.
...
image:grid_federation_remove_connection.png["procédure de suppression de la connexion de fédération de grille"]
