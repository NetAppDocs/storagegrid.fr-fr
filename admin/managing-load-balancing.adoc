...
Si vous prévoyez d'utiliser des requêtes de type hébergement virtuel S3, le certificat doit également inclure une entrée *Nom alternatif* pour chaque link:../admin/configuring-s3-api-endpoint-domain-names.html["Nom du domaine du terminal S3"] que vous avez configuré, y compris tout nom générique. Par exemple :
