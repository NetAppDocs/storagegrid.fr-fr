...
---
= Créez des applications d'entreprise dans Entra ID pour SAML dans StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Utilisez Entra ID pour créer une application d'entreprise pour chaque nœud d'administration pour SAML dans votre système.
...
* Vous avez link:../admin/configure-sso-saml.html#enter-sandbox-mode["entré en mode bac à sable"] dans Grid Manager.
...
== Étape 2 : Créez des applications d’entreprise et enregistrez la configuration SSO de StorageGRID
...
== Étape 3 : Téléchargez les métadonnées SAML pour chaque nœud d’administration
...
== Étape 4 : Charger les métadonnées SAML dans chaque application d'entreprise
...
. link:../admin/configure-sso-saml.html#test-sso["Tester chaque application"].
