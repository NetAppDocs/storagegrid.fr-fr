...
* Le service d'équilibrage de charge utilise les coûts de liaison sur les nœuds d'administration et les nœuds de passerelle pour diriger les connexions client. Reportez-vous à link:../admin/managing-load-balancing.html["Considérations relatives à l'équilibrage de charge"].


Le diagramme montre une grille à trois sites avec des coûts de liaison configurés entre les sites :

image:link_costs.png["Schéma conceptuel des coûts de liaison entre les centres de données"]
...
. Pour chaque site listé dans la colonne Coût du site/lien, saisissez une valeur de coût de 0 à 100 pour les autres sites.
+
...
+
** Pour annuler les modifications, sélectionnez *Effacer les modifications*.
** Pour enregistrer les modifications, sélectionnez *Enregistrer*.
