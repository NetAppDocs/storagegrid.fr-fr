...
summary: 'Vous pouvez configurer des groupes haute disponibilité pour fournir un accès haute disponibilité aux services sur des nœuds d"administration ou de passerelle.' 
---
...
+
image:ha_group_add_interfaces.png["Les groupes HAUTE DISPONIBILITÉ ajoutent des interfaces"]
...
+
image:vlan_parent_interface_tooltip.png["Info-bulle de l'interface VLAN"]
...
+
image:ha_group_determine_failover.png["Les groupes HAUTE DISPONIBILITÉ déterminent l'ordre de priorité"]
...
+
image:ha_group_select_virtual_ips.png["Les groupes HAUTE DISPONIBILITÉ entrent des VIP"]
