...
Si vous configurez des points de terminaison d’équilibrage de charge pour un link:../admin/configure-global-namespace.html["espace de noms global"], utilisez le même port pour chaque point de terminaison.
...
+
image:load_balancer_endpoint_details.png["Détails du terminal de l'équilibreur de charge"]
...
.. Sélectionnez l’icône de modification image:icon_edit_tm.png["Icône Modifier"].
.. Saisissez le nouveau nom.
.. Sélectionnez *Enregistrer*.
