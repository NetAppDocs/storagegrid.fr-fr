...
summary: 'Il se peut que vous ne puissiez pas vous connecter à Grid Manager si le système d"authentification unique (SSO) est indisponible. Dans ce cas, vous pouvez désactiver puis réactiver temporairement le SSO pour un Admin Node. Pour désactiver puis réactiver le SSO, vous devez accéder au shell de commande du nœud.' 
---
...
. En fonction de votre configuration SSO, exécutez l'une des commandes suivantes :
+
[role="tabbed-block"]
====
.SAML
