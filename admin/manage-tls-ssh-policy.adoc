...
+
image:securitysettings_tls_ssh_policies_current.png["Règles TLS et SSH"]
...
.Norme FIPS stricte
--
Utilisez la politique FIPS stricte si vous avez besoin de la certification Critères communs et devez utiliser le NetApp Cryptographic Security Module (NCSM) 3.1.2 ou le module NetApp StorageGRID Kernel Crypto API 6.12.63-1+ntap1 pour les connexions client externes aux points de terminaison de l'équilibreur de charge, au Tenant Manager et au Grid Manager. L'utilisation de cette politique peut réduire les performances.

Le module NCSM 3.1.2 et NetApp StorageGRID Kernel Crypto API 6.12.63-1+ntap1 sont utilisés dans les opérations suivantes :
...
+
Pour plus d'informations, consultez le programme de validation des algorithmes cryptographiques du NIST link:https://csrc.nist.gov/projects/cryptographic-algorithm-validation-program/details?validation=40426["Certificat n° A7816"^].
...
+
*Remarque* : Ce module n’a pas été soumis pour certification.



+
Pour plus d'informations, consultez le programme de validation des algorithmes cryptographiques du NIST link:https://csrc.nist.gov/projects/cryptographic-module-validation-program/entropy-validations/certificate/223["Certificat d'entropie n° E223"^].
...
Si votre StorageGRID est soumis aux exigences de cryptographie FIPS 140, vous pouvez activer la fonction de mode FIPS pour utiliser le module NCSM 3.1.2 et le module NetApp StorageGRID Kernel Crypto API 6.12.63-1+ntap1 :
...
+
image:securitysettings_custom_security_policy_copy.png["copie d'une stratégie existante pour créer une stratégie personnalisée"]
