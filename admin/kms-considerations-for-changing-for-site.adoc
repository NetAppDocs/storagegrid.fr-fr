...
summary: 'Chaque cluster de serveur de gestion des clés (KMS) ou KMS fournit une clé de chiffrement à tous les nœuds d"appliance sur un site unique ou dans un groupe de sites. Si vous devez modifier le KMS utilisé pour un site, vous devrez peut-être copier la clé de chiffrement d"un KMS vers un autre.' 
---
...
+
image:kms_default_key.png["Clé par défaut DES KMS"]
...
+
image:kms_wrong_key.png["Clé KMS erronée"]
...
+
image:kms_copied_key.png["Clé copiée DE KMS"]
