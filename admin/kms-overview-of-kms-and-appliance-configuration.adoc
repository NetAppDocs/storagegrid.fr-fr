...
summary: 'Avant de pouvoir utiliser un serveur de gestion de clés (KMS) pour sécuriser les données StorageGRID sur les nœuds de l"appliance, vous devez effectuer deux tâches de configuration : la mise en place d"un ou plusieurs serveurs KMS et l"activation du chiffrement des nœuds de l"appliance. Une fois ces deux tâches de configuration terminées, le processus de gestion des clés s"exécute automatiquement.' 
---
