...
summary: 'Avant de pouvoir utiliser un serveur de gestion de clés (KMS) pour sécuriser les données StorageGRID sur les nœuds de l"appliance, vous devez configurer un ou plusieurs serveurs KMS et activer le chiffrement des nœuds de l"appliance. Une fois ces deux tâches de configuration terminées, le processus de gestion des clés s"effectue automatiquement.' 
---
...
image:kms_configuration_overview.png["Workflow de configuration du système KM, décrit dans le texte suivant"]
