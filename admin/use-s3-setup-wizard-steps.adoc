...
. Définissez l'accès racine du compte de tenant, selon que votre système StorageGRID utilise link:using-identity-federation.html["fédération des identités"] link:how-sso-works.html["Authentification unique (SSO)"]ou les deux.
+
[cols="1a,2a"]
|===
| Option | Faites ça 
