...
* Si vous souhaitez autoriser un locataire S3 à synchroniser les données de compte et à répliquer les objets du compartiment vers une autre grille à l'aide d'une connexion de fédération de grilles :
+
...
Connexion de fédération de grille
 a| 
Le locataire peut utiliser des connexions de fédération de grille, qui :

** Cela permet de synchroniser ce compte locataire ainsi que tous les groupes de locataires, utilisateurs et clés d'accès S3 entre cette grille et les autres grilles des connexions sélectionnées.
+

TIP: Si nécessaire pour des raisons de sécurité ou de conformité, vous pouvez désélectionner *Activer la synchronisation des clés d'accès*.
...
. Si vous avez sélectionné l'autorisation *Utiliser la connexion de fédération de grilles* pour le locataire, vérifiez qu'un locataire identique a bien été répliqué sur l'autre grille de la connexion. Les locataires des deux grilles auront le même ID de compte à 20 chiffres, le même nom, la même description, le même quota et les mêmes autorisations.
+

NOTE: Si vous voyez un message d'erreur indiquant que le compte du locataire a été créé sur cette grille mais n'a pas été synchronisé avec les autres grilles, reportez-vous aux instructions dans link:grid-federation-troubleshoot.html["Dépanner les erreurs de fédération de grille"].
...
Suivez les instructions dans link:../tenant/index.html["Utilisez un compte de locataire"] pour gérer les groupes de locataires et les utilisateurs, les clés d'accès S3, les compartiments, les services de plateforme, la synchronisation des comptes et la réplication inter-grilles.
