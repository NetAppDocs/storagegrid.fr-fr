...
StorageGRID prend en charge plusieurs fournisseurs d'identité SSO (IdP).

[role="tabbed-block"]
====
.OIDC
--
Les SSO suivants IdPs sont pris en charge pour OpenID Connect (OIDC) :
...
.SAML
--
Les SSO suivants IdPs sont pris en charge pour le langage de balisage d'assertion de sécurité 2.0 (SAML 2.0) :
