...
summary: 'La réplication inter-grilles est la réplication automatique d"objets entre des compartiments S3 sélectionnés dans deux systèmes StorageGRID connectés par une fédération de grilles. La synchronisation des comptes est requise pour la réplication inter-grilles.' 
---
...
[role="lead"]
La réplication inter-grilles est la réplication automatique d'objets entre des compartiments S3 sélectionnés dans deux systèmes StorageGRID connectés dans un link:grid-federation-overview.html["connexion de fédération de grille"]. link:grid-federation-what-is-account-sync.html["Synchronisation des comptes"] est requis pour la réplication inter-grilles.
...
image:grid_federation_cgr_workflow.png["Flux de production de réplication entre les grilles"]
...
Si un compte locataire dispose de l'autorisation *Utiliser la connexion de fédération de grille* pour utiliser une ou plusieurs link:grid-federation-overview.html["connexions de fédération de grille"], un utilisateur locataire disposant de l'autorisation d'accès Root peut créer des compartiments (« source » et « destination ») dans les comptes locataires correspondants sur chaque grille. Ces compartiments :
...
* Doit disposer de l'une des configurations de verrouillage d'objet S3 suivantes :
+
** source : *désactivé*, destination : *désactivé*
** source : *activée*, destination : *activée*
** source : *désactivé*, destination : *activé*




Une fois que vous avez créé les deux compartiments, vous pouvez configurer la réplication inter-grilles pour l’un ou les deux compartiments.
...
image:grid_federation_cross_grid_replication_one_direction.png["image montrant la connexion de fédération de grille dans une direction"]
...
image:grid_federation_cross_grid_replication.png["image montrant la réplication dans une direction par rapport à la réplication dans les deux directions"]
...
image:grid_federation_cross_grid_replication_delete.png["image montrant la suppression du client répliqué sur les deux grilles"]
...
CAUTION: Vous pouvez activer le verrouillage d'objets S3 sur des compartiments existants, ce qui pourrait entraîner une mauvaise configuration ultérieure.


NOTE: Lorsque le verrouillage d'objets S3 est activé sur le compartiment source, assurez-vous qu'il l'est également sur le compartiment de destination. Si le verrouillage d'objets S3 est désactivé sur le compartiment de destination, une erreur se produit lors de la réplication. Consultez link:../admin/grid-federation-troubleshoot.html#cross-grid-replication-alerts-and-errors["Alertes et erreurs de réplication intergrid"].
