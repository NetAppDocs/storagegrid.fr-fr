...
Vous pouvez importer un ou plusieurs utilisateurs fédérés, jusqu'à 100 utilisateurs maximum, directement dans la page utilisateurs.

.Étapes
. Sélectionnez *CONFIGURATION* > *utilisateurs Admin*.
. Sélectionnez *Importer les utilisateurs fédérés*.
. Entrez l'UUID ou le nom d'utilisateur d'un ou plusieurs utilisateurs fédérés.
+
Pour plusieurs entrées, ajoutez chaque UUID ou nom d'utilisateur sur une nouvelle ligne.

. Sélectionnez *Importer*.
+
Si l'importation dans le champ utilisateurs échoue pour un ou plusieurs utilisateurs, effectuez les opérations suivantes :

+
.. Sélectionnez *Copier utilisateurs*.
.. Relancez l'importation en sélectionnant *Précédent* et en collant les utilisateurs copiés dans la boîte de dialogue *Importer les utilisateurs fédérés*.


+
Après avoir fermé la boîte de dialogue *Importer les utilisateurs fédérés*, les informations relatives à l'utilisateur fédéré s'affichent sur la page utilisateurs pour les utilisateurs correctement importés.
