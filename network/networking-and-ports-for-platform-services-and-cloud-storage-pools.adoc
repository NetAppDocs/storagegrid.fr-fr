...
Les pools de stockage cloud nécessitent également un accès depuis les nœuds de stockage aux points de terminaison fournis par le service externe utilisé, tel qu'Amazon S3 Glacier ou Microsoft Azure Blob Storage. Pour plus d'informations, voir link:../ilm/what-cloud-storage-pool-is.html["Qu'est-ce qu'un pool de stockage cloud"].
