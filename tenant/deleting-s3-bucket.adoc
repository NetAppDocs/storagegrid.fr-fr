...
Vous ne pouvez pas supprimer un compartiment S3 s'il contient des objets, des versions d'objets obsolètes ou des marqueurs de suppression. Pour plus d'informations sur la suppression des objets versionnés S3, consultez link:../ilm/how-objects-and-delete-markers-are-deleted.html["Comment les objets et les marqueurs de suppression sont supprimés"].
