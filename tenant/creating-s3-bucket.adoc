...
* Si plusieurs locataires disposent chacun de jusqu'à 5 000 compartiments, chaque nœud de stockage de la grille possède un minimum de 64 Go de RAM.
+

NOTE: Le système StorageGRID prend en charge un maximum de 20 000 compartiments par compte locataire S3. Chaque grille peut contenir jusqu'à 100 000 compartiments, y compris link:../tenant/what-is-branch-bucket.html["seaux de branches"]. Il existe également une limite pour link:../tenant/manage-branch-buckets.html["profondeur du compartiment de branche"].
...
Pour plus d'informations, veuillez consulter la  https://docs.aws.amazon.com/AmazonS3/latest/userguide/bucketnamingrules.html["Documentation Amazon Web Services (AWS) sur les règles d'attribution de nom de compartiment"^].
