...
image:grid-federation-account-clone-workflow-tm.png["workflow du locataire pour le clone du compte. Les étapes sont décrites dans le texte suivant."]
...
image:grid-federation-account-clone.png["image montrant que les groupes locaux sont clonés de la grille source à la grille de destination"]
...
image:grid-federation-local-user-clone.png["image montrant que les utilisateurs locaux sont clonés de la grille source à la grille de destination"]
...
image:grid-federation-federated-group-clone.png["image montrant que les groupes fédérés sont clonés de la grille source à la grille de destination"]
...
image:grid-federation-s3-access-key.png["image montrant que les clés d'accès s3 peuvent être clonées entre la grille source et la grille de destination"]
...
image:grid-federation-account-not-cloned.png["image montrant que les détails de la grille de destination ne sont pas clonés dans la grille source"]
...
image:grid-federation-account-clone-edit-delete.png["l'image montrant que les détails modifiés ou supprimés ne sont pas clonés"]
