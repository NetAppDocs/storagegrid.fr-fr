...
summary: 'Si l"autorisation Use grid federation connection a été attribuée à votre compte locataire lors de sa création, vous pouvez utiliser la réplication inter-grilles pour répliquer automatiquement les objets entre les compartiments de la grille source du locataire et ceux de la grille de destination. La réplication inter-grilles peut s"effectuer dans un sens ou dans les deux.' 
---
...
+
image:grid_federation_cross_grid_replication_client_deletes.png["image montrant la suppression du client répliqué sur les deux grilles"]
