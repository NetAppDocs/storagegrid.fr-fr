...
Les services de plate-forme communiquent avec les noeuds finaux externes que vous configurez à l'aide du link:configuring-platform-services-endpoints.html["Gestionnaire de locataires"]ou du link:understanding-tenant-management-api.html["API de gestion des locataires"]. Chaque terminal représente une destination externe, par exemple un compartiment StorageGRID S3, un compartiment Amazon Web Services, une rubrique Amazon SNS, un terminal Webhook ou un cluster Elasticsearch hébergé localement, sur AWS ou ailleurs.
