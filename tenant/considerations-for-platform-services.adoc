...
Il est possible de configurer n'importe quelle combinaison de services de plateforme pour un seul compartiment S3. Par exemple, vous pouvez configurer à la fois link:../tenant/understanding-cloudmirror-replication-service.html["Service CloudMirror"] et link:../tenant/understanding-notifications-for-buckets.html["notifications"] sur un compartiment StorageGRID S3 afin de mettre en miroir des objets spécifiques vers Amazon Simple Storage Service (S3), tout en envoyant une notification concernant chaque objet à une application de surveillance tierce pour vous aider à suivre vos dépenses AWS.
...
Compartiment avec verrouillage objet S3 activé
 a| 
Si le compartiment S3 source dans StorageGRID a S3 Object Lock activé, vous devez également activer S3 Object Lock pour le compartiment de destination avec la rétention par défaut configurée.

La réplication des _données d'objet_ est prise en charge pour les compartiments source et de destination avec S3 Object Lock activé. Toutefois, les compartiments de destination conserveront leurs propres paramètres de rétention des objets et peuvent ne pas proposer la conformité ou le versionnement.
