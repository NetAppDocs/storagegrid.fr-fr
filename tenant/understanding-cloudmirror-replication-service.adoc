...
NOTE: CloudMirror replication est prise en charge uniquement si le compartiment source a le verrouillage d'objet S3 activé et si le point de terminaison est un compartiment Amazon ou StorageGRID S3. Les paramètres de verrouillage d'objet ne sont pas répliqués vers le point de terminaison du compartiment.
