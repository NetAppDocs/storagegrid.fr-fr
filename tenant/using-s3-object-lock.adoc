...
Lorsque le paramètre global de verrouillage d'objets S3 est activé pour un système StorageGRID, un compte locataire S3 peut créer des compartiments avec ou sans le verrouillage d'objets S3 activé. Lorsque l'option permettant d'autoriser le verrouillage d'objets S3 pour les compartiments existants est activée, un compte locataire S3 peut activer le verrouillage d'objets S3 pour un compartiment existant. Si un compartiment a le verrouillage d'objets S3 activé, le versionnage du compartiment est requis et est activé automatiquement. Après qu'un compte locataire S3 a activé le verrouillage d'objets S3 pour un compartiment nouveau ou existant, il ne peut plus être désactivé.
...
+
Pour plus de détails sur les paramètres de l'objet, reportez-vous à link:../s3/use-s3-api-for-s3-object-lock.html["Utilisez l'API REST S3 pour configurer le verrouillage objet S3"].
...
Reportez-vous à link:../tenant/creating-s3-bucket.html["Créer un compartiment S3"] et link:../tenant/update-default-retention-settings.html["Mettre à jour la conservation par défaut du verrouillage d'objet S3"].
...
* Autoriser les utilisateurs à activer le verrouillage d'objets S3 pour les compartiments existants.
...
* Au besoin, contactez un administrateur de grille pour activer l'un des paramètres suivants :
+
** Verrouillage d'objet global S3
** Autoriser l'activation du verrouillage d'objets S3 pour les compartiments existants


* Créez des compartiments avec le verrouillage d'objet S3 activé.
* Activez le verrouillage des objets S3 sur les compartiments existants.
...
Apprenez-en plus sur link:../ilm/requirements-for-s3-object-lock.html#s3-object-lock-existing-buckets["Exigences de verrouillage d'objet S3"]
