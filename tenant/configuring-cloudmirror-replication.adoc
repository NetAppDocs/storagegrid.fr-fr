...
* La réplication est uniquement prise en charge pour les compartiments source ou de destination dont le verrouillage d'objet S3 est activé pour les compartiments Amazon S3 ou StorageGRID S3. Les en-têtes de verrouillage d'objet ne sont pas répliqués vers l'objet de destination.
...
* Si vous activez le verrouillage d'objet S3 sur le compartiment de destination après avoir configuré CloudMirror la réplication, vous devrez peut-être link:../tenant/editing-platform-services-endpoint.html["enregistrer à nouveau le point de terminaison"] pour autoriser la reprise de la réplication.
