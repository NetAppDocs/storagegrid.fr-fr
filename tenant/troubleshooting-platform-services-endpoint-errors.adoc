...
. Sur la page de détails du point de terminaison, sélectionnez *Connection*. Cet onglet affiche uniquement l’erreur la plus récente pour un point de terminaison et indique depuis combien de temps elle s’est produite. Les erreurs comportant l’icône X rouge image:icon_alert_red_critical.png["Icône X rouge"] se sont produites au cours des 7 derniers jours.
