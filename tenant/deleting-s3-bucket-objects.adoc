...
* Si le versionnement d'objets est activé sur un compartiment, l'opération de suppression d'objets ne retirera pas les marqueurs de suppression créés dans StorageGRID 11.7 ou version antérieure. Consultez les informations relatives à la suppression d'objets dans un compartiment dans link:../ilm/how-objects-and-delete-markers-are-deleted.html#delete-s3-versioned-objects["Suppression d'objets avec version S3"].
...
+
image:delete_bucket_objects_in_progress.png["capture d'écran montrant la suppression d'objets de compartiment"]
...
+
image:delete_bucket_objects_complete.png["capture d'écran montrant tous les objets de compartiment supprimés"]
