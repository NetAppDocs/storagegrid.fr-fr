...
Le client S3 effectue une requête (SUPD) pour mettre à jour les métadonnées spécifiées (`x-amz-meta-*` pour l'objet S3 (S3KY).

Dans cet exemple, les en-têtes de requête sont inclus dans le champ HTRH car celui-ci a été configuré comme en-tête de protocole d'audit (*Configuration* > *Monitoring* > *Audit and syslog server*). Voir link:../monitor/configure-log-management.html["Configurer la gestion des journaux et le serveur syslog externe"].
