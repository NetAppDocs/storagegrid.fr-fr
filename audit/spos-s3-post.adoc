...
ASRS
 a| 
URN de session de rôle assumé
 a| 
L'identifiant unique de la session de rôle assumé.

Présent uniquement lorsque la requête S3 utilise des informations d'identification de sécurité temporaires provenant d'une requête AssumeRole.
...
URN de rôle assumé
 a| 
L'identifiant unique du rôle assumé.
