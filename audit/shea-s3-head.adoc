...
summary: 'Lorsqu"un client S3 émet une opération HEAD, une demande est formulée pour vérifier l"existence d"un objet ou d"un compartiment et extraire les métadonnées sur un objet. Ce message est émis par le serveur si l"opération a réussi.' 
---
= SHEA : TÊTE S3
:allow-uri-read: 
:icons: font
...
[role="lead"]
Lorsqu'un client S3 émet une opération HEAD, une demande est formulée pour vérifier l'existence d'un objet ou d'un compartiment et extraire les métadonnées sur un objet. Ce message est émis par le serveur si l'opération a réussi.
