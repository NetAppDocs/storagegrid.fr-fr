...
summary: 'L"API REST S3 utilisée par StorageGRID est mise à jour régulièrement.' 
---
= Modifications apportées à la prise en charge de l'API REST S3 de StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
L'API REST S3 utilisée par StorageGRID est mise à jour régulièrement.
...
* Ajout de la prise en charge des valeurs de somme de contrôle précalculées pour les requêtes et en-têtes suivants :
+
** CompleteMultipartUpload: `x-amz-checksum-crc32`, `x-amz-checksum-crc32c`, et `x-amz-checksum-crc64nvme`
** CreateMultipartUpload: `CRC32`, `CRC32C` et  `CRC64NVME` pour  `x-amz-checksum-algorithm`
** CopyObject: `CRC32`, `CRC32C`, `CRC64NVME`, et `SHA256` pour `x-amz-checksum-algorithm`
** PutObject: `x-amz-checksum-crc32`, `x-amz-checksum-crc32c` et `x-amz-checksum-crc64nvme`
** UploadPart: `x-amz-checksum-crc32`, `x-amz-checksum-crc32c` et `x-amz-checksum-crc64nvme`


* Ajout de la prise en charge de  `x-amz-checksum-type` pour CreateMultipartUpload et CompleteMultipartUpload.
* Ajout de la prise en charge de la clé de condition de stratégie  `s3:x-amz-copy-source`.
* Autoriser l'activation du verrouillage d'objets S3 sur les compartiments existants.
...
* Ajout de la prise en charge de STS AssumeRole et de la stratégie de session. Voir link:../s3/example-session-policies.html["un exemple de politique de session"]. Vous pouvez configurer AssumeRole dans les groupes de locataires.
...
11,7
 a| 
* Ajouté link:../s3/quick-reference-support-for-aws-apis.html["Référence rapide : demandes d'API S3 prises en charge"].
...
10,1
 a| 
Ajout de la prise en charge du téléchargement en plusieurs parties, des requêtes de type hébergement virtuel et de l'authentification v4.
...
|===
Consultez link:../s3/index.html#supported-versions["Versions et mises à jour prises en charge de l’API REST S3"] pour plus d'informations.
