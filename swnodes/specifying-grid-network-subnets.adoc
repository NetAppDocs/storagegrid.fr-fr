...
---
= Spécifiez les sous-réseaux du réseau Grid pour StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
. Sélectionnez le signe plus à côté de la dernière entrée pour ajouter une entrée réseau supplémentaire. Vous devez spécifier tous les sous-réseaux pour tous les sites du réseau Grid.
+
** Si vous avez déjà déployé au moins un nœud, sélectionnez *Discover Grid Networks subnets* pour remplir automatiquement la liste des sous-réseaux Grid Network avec les sous-réseaux signalés par les nœuds de grille enregistrés auprès du Grid Manager.
...
+
image:4_gmi_installer_grid_network_page.gif["Spécifiez les sous-réseaux"]
