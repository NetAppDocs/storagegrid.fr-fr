...
---
= Spécifiez les informations du serveur Network Time Protocol pour StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
https://learn.microsoft.com/en-us/troubleshoot/windows-server/active-directory/support-boundary-high-accuracy-time["Limite de support pour une heure de haute précision"^]
...
(VMware uniquement) Effectuez des vérifications supplémentaires pour VMware, par exemple en vous assurant que l'hyperviseur utilise la même source NTP que la machine virtuelle et en utilisant VMTools pour désactiver la synchronisation horaire entre l'hyperviseur et les machines virtuelles StorageGRID .
...
+
image:8_gmi_installer_ntp_page.gif["Cette image est expliquée par le texte qui l'entoure."]
