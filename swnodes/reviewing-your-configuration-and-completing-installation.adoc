...
---
= Vérifiez la configuration de votre StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
image:11_gmi_installer_summary_page.gif["Page récapitulative du programme d'installation de Grid Manager"]
...
NOTE: Si un nœud est configuré pour utiliser le réseau client, la passerelle par défaut de ce nœud bascule du réseau Grid vers le réseau client lorsque vous sélectionnez *Installer*. Si vous perdez la connectivité, vous devez vous assurer que vous accédez au nœud d'administration principal via un sous-réseau accessible. Voir link:../network/index.html["Instructions de mise en réseau"] pour plus de détails.
...
. Cochez la case *J'ai téléchargé et vérifié avec succès le fichier du Recovery Package*, puis sélectionnez *Suivant*.
+
...
+
image:12_gmi_installer_status_page.gif["Cette image est expliquée par le texte qui l'entoure."]
