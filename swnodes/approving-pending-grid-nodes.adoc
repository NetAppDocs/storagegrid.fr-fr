...
---
= Approuver les nœuds de grille en attente pour StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
image:5_gmi_installer_grid_nodes_pending.gif["Cette image est expliquée par le texte qui l'entoure."]
...
.. Retournez à la page d'accueil et sélectionnez *Démarrer l'installation*.
...
+
image:7_gmi_installer_grid_nodes_approved.gif["Cette image est expliquée par le texte qui l'entoure."]

. Répétez ces étapes pour chaque nœud de grille en attente à approuver.
+
Vous devez approuver tous les nœuds que vous souhaitez dans la grille. Cependant, vous pouvez revenir à cette page à tout moment avant de sélectionner *Installer* sur la page Résumé. Vous pouvez modifier les propriétés d'un nœud de grille approuvé en sélectionnant son bouton radio et en sélectionnant *Modifier*.

. Une fois que vous avez terminé d'approuver les nœuds de la grille, sélectionnez *Suivant*.
