...
.Étapes
. Dans *Phrase secrète de provisionnement*, saisissez la phrase secrète de provisionnement qui sera nécessaire pour apporter des modifications à la topologie de la grille de votre système StorageGRID.
+
...
+
image:10_gmi_installer_passwords_page.gif["Cette image est expliquée par le texte qui l'entoure."]
...
NOTE: Vous êtes invité à télécharger le fichier Recovery Package (`sgws-recovery-package-id-revision.zip` après avoir sélectionné *Installer* sur la page Résumé. Vous devez link:../maintain/downloading-recovery-package.html["téléchargez ce fichier"] pour terminer l'installation. Les mots de passe nécessaires pour accéder au système sont stockés dans le fichier  `Passwords.txt`, contenu dans le fichier Recovery Package.
