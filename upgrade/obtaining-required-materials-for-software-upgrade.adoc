...
---
= Obtenez les éléments nécessaires pour une mise à niveau du logiciel StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Documentation StorageGRID
 a| 
* https://library.netapp.com/ecm/ecm_download_file/ECMLP3373065["Notes de mise à jour"^]. Vous devez vous connecter avec votre compte NetApp ou en créer un pour accéder aux notes de version. Veuillez les lire attentivement avant de procéder à la mise à niveau.
* https://kb.netapp.com/hybrid/StorageGRID/Maintenance/StorageGRID_12.1_software_upgrade_resolution_guide["Guide de résolution des mises à niveau logicielles StorageGRID"^] pour la version majeure vers laquelle vous effectuez la mise à niveau (connexion requise)
