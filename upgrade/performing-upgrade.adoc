...
summary: Vous pouvez mettre à niveau vers StorageGRID 12.1 et appliquer le correctif le plus récent pour cette version en même temps. La page de mise à niveau de StorageGRID fournit le chemin de mise à niveau recommandé et des liens directs vers les pages de téléchargement appropriées. 
---
= Effectuez une mise à niveau de StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Vous pouvez mettre à niveau vers StorageGRID 12.1 et appliquer le correctif le plus récent pour cette version en même temps. La page de mise à niveau de StorageGRID fournit le chemin de mise à niveau recommandé et des liens directs vers les pages de téléchargement appropriées.
...
Le chemin de mise à jour sur la page de mise à niveau StorageGRID indique quelles versions majeures (par exemple, 12.1.0) et quels correctifs (par exemple, 12.1.0.1) vous devez installer pour obtenir la dernière version de StorageGRID. Vous devez installer les versions et les correctifs recommandés dans l'ordre indiqué.
...
. Sélectionnez *Parcourir* pour télécharger le fichier de mise à niveau de version sur StorageGRID : `NetApp_StorageGRID_12.1.0_Software_uniqueID.upgrade`
+
...
. Examinez les détails de chaque vérification préalable signalée et corrigez toute erreur signalée. Consultez la  https://kb.netapp.com/hybrid/StorageGRID/Maintenance/StorageGRID_12.1_software_upgrade_resolution_guide["Guide de résolution des mises à niveau logicielles StorageGRID"^] pour la version StorageGRID 12.1.
+
...
NOTE: Pour une mise à niveau de StorageGRID 12.1, si vous souhaitez approuver des nœuds individuels au lieu de tous les nœuds, la bonne pratique consiste à mettre à niveau un site entier avant de passer au site suivant.
...
.Étapes
. Dès que la tâche *Activer les fonctionnalités* est terminée (jusqu'à une heure), vous pouvez commencer à utiliser la link:whats-new.html["nouvelles fonctionnalités"] dans la version mise à jour de StorageGRID.
+
