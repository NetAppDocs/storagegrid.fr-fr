...
---
= Configurer StorageGRID ILM pour les données FabricPool
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Si vous avez effectué une mise à niveau vers StorageGRID 12.1 depuis une version de StorageGRID antérieure à 11.7, vous avez configuré le pool de stockage que vous utiliserez. En règle générale, vous devez créer un pool de stockage pour chaque site StorageGRID que vous utiliserez pour stocker des données. (À partir de la version 11.7, les pools de stockage sont créés automatiquement pour chaque site.)
+
