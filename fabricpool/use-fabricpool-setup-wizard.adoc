...
---
= Découvrez l'assistant de configuration FabricPool de StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Si vous prévoyez d’utiliser des requêtes S3 de type hébergement virtuel, vous avez link:../admin/configuring-s3-api-endpoint-domain-names.html["Noms de domaine de terminaux S3 configurés"]. ONTAP utilise par défaut des URL de type chemin, mais il est recommandé d’utiliser des requêtes de type hébergement virtuel.
