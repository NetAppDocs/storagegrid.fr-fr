...
---
= Configurez ONTAP System Manager pour ajouter StorageGRID en tant que niveau cloud FabricPool
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Style d'URL
 a| 
Si vous link:../admin/configuring-s3-api-endpoint-domain-names.html["Noms de domaine de terminaux S3 configurés"], sélectionnez *URL de type hébergement virtuel*.

*Path-Style URL* est le format par défaut pour ONTAP, mais l'utilisation de requêtes de type virtual-hosted-style est recommandée pour StorageGRID. Vous devez utiliser *Path-Style URL* si vous indiquez une adresse IP au lieu d'un nom de domaine dans le champ *Server name (FQDN)*.
