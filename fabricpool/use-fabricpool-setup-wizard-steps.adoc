...
. Si la bannière *FabricPool and S3 setup wizard* apparaît sur le tableau de bord, sélectionnez le lien dans la bannière. Si la bannière n'apparaît plus, sélectionnez l'icône d'aide image:icon_help_menu_bar.png["Icône d'aide"] dans la barre d'en-tête du Grid Manager et sélectionnez *FabricPool and S3 setup wizard*.
...
.Étapes
. Vous pouvez également sélectionner l’icône de copie (image:icon_tenant_copy_url.png["icône copier"]) pour copier à la fois l’ID de clé d’accès et la clé d’accès secrète dans le presse-papiers.
+
