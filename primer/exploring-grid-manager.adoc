...
image:grid_manager_dashboard_and_menu.png["Tableau de bord dans Grid Manager"]

Pour obtenir une explication des informations affichées sur chaque carte, sélectionnez l'icône d'aide image:icon_nms_question.png["icône point d'interrogation"] pour cette carte.
...
Le menu d'aide image:icon_help_menu_bar.png["icône d'aide dans la barre de menus"] donne accès à :
...
* Utilisez link:../admin/grid-federation-overview.html["fédération des grilles"] pour synchroniser les informations des comptes locataires et répliquer les données d’objets entre deux systèmes StorageGRID
...
* link:../monitor/configure-email-server.html["Configurer le serveur de messagerie"]
...
* link:../admin/about-multi-admin-verify.html["Configurer la vérification multi-administrateur"]
...
* Restauration des données d'objet
+
...
* link:../monitor/configure-autosupport-grid-manager.html["Configurez AutoSupport"]
