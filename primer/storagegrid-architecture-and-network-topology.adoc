...
Pour plus d'informations sur la topologie du réseau StorageGRID, les exigences et les communications de la grille, veuillez consulter link:../network/index.html["Instructions de mise en réseau"].
...
image:data_center_site_single.png["Un seul data Center site"]
...
image:data_center_sites_multiple.png["Site de plusieurs data centers"]
...
image:grid_nodes_and_components.png["schéma décrit par le texte environnant"]

Les clients S3 stockent et récupèrent des objets dans StorageGRID. D'autres clients sont utilisés pour accéder à l'interface de gestion de StorageGRID et, en option, au partage d'audit. De plus, StorageGRID peut, en option, envoyer des alertes par e-mail via un serveur SMTP externe.
...
Les objets peuvent être stockés dans StorageGRID sur des nœuds de stockage logiciels ou matériels, ou dans des pools de stockage cloud, qui consistent en des compartiments S3 externes ou des conteneurs Azure Blob Storage.
