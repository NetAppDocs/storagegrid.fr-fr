...
Par défaut, tous les nouveaux nœuds de stockage stockent à la fois les données d'objet et les métadonnées. Ce type de nœud de stockage est appelé un nœud de stockage _combiné_.
...
* Lorsque vous utilisez des appliances StorageGRID, vous ne pouvez configurer que des nœuds uniquement de métadonnées sur les appliances SGF6112 avec douze disques de 1,9 To ou douze disques de 3,8 To.
* Lors de l'utilisation de nœuds logiciels, vous devez vous assurer que les ressources des nœuds uniquement de métadonnées correspondent aux ressources des nœuds de stockage existants. Par exemple :
+
...
Vous pouvez convertir un nœud de stockage combiné en un nœud de stockage dédié aux données. Si le nœud de stockage contient le <<adc-service,Service ADC>> et que les conditions préalables à la conversion sont remplies, le service est automatiquement déplacé vers un autre nœud de stockage du même site pendant la conversion. Pour connaître les conditions préalables et la procédure détaillée, consultez link:../maintain/convert-to-data-only-node.html["Convertir un nœud de stockage en nœud de données uniquement"].
...
Le service ADC gère les informations de topologie, notamment la localisation et la disponibilité des services. Lorsqu’un nœud du grid a besoin d’informations provenant d’un autre nœud du grid ou d’une action à effectuer par un autre nœud du grid, il contacte un service ADC pour trouver le meilleur nœud du grid afin de traiter sa requête. De plus, le service ADC conserve une copie des bundles de configuration du déploiement StorageGRID, permettant à tout nœud du grid de récupérer les informations de configuration actuelles.
