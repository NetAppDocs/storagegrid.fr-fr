...
image:replication_data_flow.png["schéma illustrant le processus décrit dans le texte environnant"]
...
image:erasure_coding_data_flow.png["Codage d'effacement des flux de données"]
...
Si les instructions de placement de contenu d'une règle ILM exigent qu'une copie répliquée des données d'objet soit stockée sur un Cloud Storage Pool, les données d'objet sont dupliquées dans le compartiment S3 externe ou le conteneur Azure Blob Storage spécifié pour le Cloud Storage Pool.
...
image:cloud_storage_pool_data_flow.png["Flux de données du pool de stockage cloud"]
