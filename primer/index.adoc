...
image:storagegrid_system_diagram.png["Image avec plusieurs clients"]
...
* Gestion fédérée des identités. S’intègre à Active Directory, OpenLDAP ou Oracle Directory Service pour l’authentification des utilisateurs. Prend en charge l’authentification unique (SSO) à l’aide du protocole Security Assertion Markup Language 2.0 (SAML 2.0) ou OpenID Connect (OIDC) pour l’échange de données d’authentification et d’autorisation entre StorageGRID et un fournisseur d’identité externe tel que Active Directory Federation Services (AD FS).
