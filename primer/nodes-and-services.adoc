...
Nœuds d'administration:: Fournissez des services de gestion tels que la configuration système, la surveillance et la journalisation. Lorsque vous vous connectez à Grid Manager, vous vous connectez à un Admin Node. Chaque grid doit avoir un Admin Node principal et peut avoir des Admin Nodes non principaux supplémentaires pour la redondance. Vous pouvez vous connecter à n'importe quel Admin Node, et chaque Admin Node affiche une vue similaire du système StorageGRID. Cependant, vous devez effectuer la plupart des procédures de maintenance à l'aide de l'Admin Node principal.
+
--
...
+
--
Lors de l'installation initiale d'un nouveau nœud de stockage, vous pouvez spécifier qu'il ne soit utilisé que pour link:../primer/what-storage-node-is.html#types-of-storage-nodes["stocker soit des métadonnées, soit des données"].
...
gdu-server
 a| 
Le gestionnaire de tâches backend pour la maintenance et le provisionnement du grid. Accepte les requêtes provenant de  `mgmt-api` et d'autres scripts utilitaires, les valide et répartit chaque action vers le gestionnaire de tâches GDU approprié, comme le provisionnement, l'expansion, la mise hors service et les mises à niveau.
 a| 
Nœuds d'administration
...
Haute disponibilité
 a| 
Gère les adresses IP virtuelles à haute disponibilité sur les nœuds configurés sur la page Groupes à haute disponibilité. Ce service est également connu sous le nom de service _keepalived_.
 a| 
...
mgmt-api
 a| 
L'API backend de Grid Manager. Expose des points de terminaison REST authentifiés pour l'administration de la grille (configuration, surveillance et maintenance), gère les validations et les autorisations, et orchestre les opérations de longue durée. Il s'agit de l'API principale du plan de contrôle utilisée par l'interface de Grid Manager et les clients d'automatisation.
 a| 
Nœuds d'administration
