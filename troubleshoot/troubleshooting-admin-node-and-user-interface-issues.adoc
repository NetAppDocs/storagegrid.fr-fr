...
---
= Résolvez les problèmes liés au nœud d'administration et à l'interface utilisateur dans StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
. Si l'authentification unique (SSO) est activée pour votre système StorageGRID, reportez-vous à la configuration de l'authentification unique à l'aide de link:../admin/learn-about-setting-up-sso-oidc.html["OIDC"] ou link:../admin/learn-about-setting-up-sso-saml.html["SAML"].
+
...
... Sélectionnez *Tester la connexion* pour valider vos paramètres de connexion au serveur LDAP.
