...
* link:../monitor/manually-triggering-autosupport-package.html["Déclencher manuellement un package AutoSupport"]
...
Vous pouvez collecter ces informations pour l'intégralité du système ou pour un data Center spécifique.
 ...
 a| 
Taux d'opérations S3
 a| 
Opérations/seconde
 a| 
Dans le tableau de bord du Grid Manager, sélectionnez *Performances* > *Opérations S3 pour les nœuds de stockage*.

Pour voir les taux et les nombres d'ingestion et de récupération pour un site ou un nœud spécifique, sélectionnez *Nœuds* > *_site ou Nœud de stockage_* > *Objets*.  Positionnez votre curseur sur le graphique d'ingestion et de récupération S3.
...
Millisecondes
 a| 
Sélectionnez *Nœuds* > *_Nœud de stockage_* > *Objets*.  Dans la table Requêtes, affichez la valeur de la latence moyenne.
...
[cols="1a,2a,4a"]
|===
| image:feature_checkmark.gif["coche"] | Élément | Remarques 
...
Fichiers journaux et données système
 a| 
Collectez les fichiers journaux et les données système de votre système.  Sélectionnez *Support* > *Outils* > *Collecte de journaux*.

Vous pouvez collecter les journaux pour toute la grille ou pour certains nœuds.

Si vous collectez des journaux uniquement pour des nœuds sélectionnés, assurez-vous d'inclure au moins un nœud de stockage doté du service ADC.  Les trois premiers nœuds de stockage installés sur un site incluent le service ADC.
