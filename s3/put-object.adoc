...
+
** StorageGRID ne vérifie pas le  `chunk-signature` par rapport aux données des blocs.
** StorageGRID ne vérifie pas la valeur que vous fournissez pour `x-amz-decoded-content-length` par rapport à l'objet.
...
NOTE: Si une requête PutObject ne spécifie pas de somme de contrôle, StorageGRID ne calcule pas de somme de contrôle CRC-64/NVME par défaut pour un objet.
