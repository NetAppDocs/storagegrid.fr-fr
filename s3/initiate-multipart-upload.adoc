...
NOTE: Si la requête CreateMultipartUpload ne spécifie pas de somme de contrôle, StorageGRID ne calcule pas de somme de contrôle CRC-64/NVME par défaut pour un objet. StorageGRID ignore les valeurs invalides ou non prises en charge `x-amz-checksum-algorithm`.
