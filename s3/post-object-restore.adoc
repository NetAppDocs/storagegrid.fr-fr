...
*Important* : Si un objet a été transféré vers S3 Glacier Deep Archive ou si le Cloud Storage Pool utilise Azure Blob Storage, vous ne pouvez pas le restaurer à l’aide du niveau  `Expedited`. L’erreur suivante est renvoyée `403 Forbidden`, `InvalidTier` : `Retrieval option is not supported by this storage class`.
