...
summary: 'StorageGRID utilise le langage de stratégie Amazon Web Services (AWS) pour permettre aux locataires S3 de contrôler l"accès aux compartiments et aux objets contenus dans ces compartiments.' 
---
...
* Les caractères internationaux, qui peuvent être spécifiés dans la clé de l'objet, doivent être encodés en JSON UTF-8 ou à l'aide des séquences d'échappement JSON \u. L'encodage en pourcentage n'est pas pris en charge.
+
...
== [[use-putoverwriteobject-permission]]Utilisez l'autorisation PutOverwriteObject
...
** Si aucun objet existant n'est trouvé, cette autorisation est sans effet.


* Lorsque cette autorisation est absente, l'effet est le même que si Allow était activé.
...
Opérateurs IP
 a| 
Comparée à l'adresse IP à partir de laquelle la requête a été envoyée. Peut être utilisée pour les opérations sur les compartiments ou les objets.

*Remarque :* Si la requête S3 a été envoyée via le service Load Balancer sur les nœuds d’administration et les nœuds de passerelle, cela correspond à l’adresse IP en amont du service Load Balancer.

*Remarque* : Si un équilibreur de charge tiers non transparent est utilisé, la comparaison porte sur l’adresse IP de cet équilibreur. Tout  `X-Forwarded-For` en-tête est ignoré car sa validité ne peut être vérifiée.
...
Ressource/identité
 a| 
Comparé au nom d'utilisateur de l'expéditeur à partir duquel la requête a été envoyée. Peut être utilisé pour les opérations sur les compartiments ou les objets.
...
s3:permissions ListBuckeVersions
 a| 
Compare au paramètre délimiteur spécifié dans une requête ListObjects ou ListObjectVersions.
...
s3:PutObjectVersionTagging
 a| 
Nécessite que l'objet existant possède la clé et la valeur de balise spécifiques.
...
s3:permissions ListBuckeVersions
 a| 
Se compare au paramètre max-keys spécifié dans une requête ListObjects ou ListObjectVersions.
...
s3:permissions ListBuckeVersions
 a| 
Se compare au paramètre de préfixe spécifié dans une requête ListObjects ou ListObjectVersions, ou à la clé spécifiée pour GetObject ou HeadObject.
...
s3:PutObjectVersionTagging
 a| 
Nécessite une clé et une valeur de balise spécifiques lorsque la requête d'objet inclut un étiquetage.
...
s3:PutObject
 a| 
Comparé à l'en-tête x-amz-copy-source développé à partir de l'en-tête de requête dans la requête PutObject et UploadPartCopy.
...
Godet
 a| 
Valide. Les objets appartiennent au compte créateur et la stratégie du compartiment ne s'applique pas. Le compte créateur doit accorder les autorisations d'accès à l'objet à l'aide des listes de contrôle d'accès (ACL) d'objet.
 a| 
...
NOTE: Le fait de définir DeleteObject sur REFUSER dans une stratégie S3 n'empêche pas ILM de supprimer des objets lorsqu'une règle telle que « zéro copie après 30 jours » existe.
