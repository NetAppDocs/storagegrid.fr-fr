...
Cette page répertorie uniquement les opérations S3 prises en charge par StorageGRID. Toute opération S3 non répertoriée peut  `501 Not Implemented` ou entraîner un comportement inattendu.


TIP: Pour afficher la documentation AWS pour chaque opération, sélectionnez le lien dans l'en-tête.
...
.Paramètres de requête URI et en-têtes de requête
StorageGRID prend en charge la _plus grande partie_ <<common-params,paramètres et en-têtes communs>> pour cette requête.

.Corps de la demande
StorageGRID prend en charge la _plupart_ des paramètres de corps de requête définis par l'API REST Amazon S3 au moment de l'implémentation, à l'exception suivante :

Vous ne pouvez pas utiliser PutObjectLockConfiguration pour activer le verrouillage d'objet S3 sur un compartiment existant, sauf si votre administrateur de grille vous y a autorisé.
