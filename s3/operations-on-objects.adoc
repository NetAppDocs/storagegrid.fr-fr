...
* Les en-têtes  `If-Match` et  `If-None-Match` se comportent différemment selon la configuration de votre StorageGRID. Fournir ces en-têtes peut entraîner un message d’erreur (501 Not Implemented). Si les en-têtes  `If-Match` ou  `If-None-Match` sont acceptés mais non fonctionnels, une alerte peut être déclenchée avec des instructions concernant l’activation du comportement d’erreur 501 Not Implemented plus strict.
