...
Compare avec le paramètre max-keys spécifié dans une requête ListObjects ou ListObjectVersions.
 ...
 a| 
s3:mode-verrouillage-objet
 a| 
s3:PutObject
 a| 
Compare avec l' `object-lock-mode`en-tête de requête développé dans la requête PutObject, CopyObject et CreateMultipartUpload.
...
s3:PutObjectRetention
 a| 
Compare au `object-lock-mode` corps XML développé dans la requête PutObjectRetention.
...
Nécessitera une clé de balise et une valeur spécifiques lorsque la demande d'objet inclut le balisage.
 ...
 a| 
s3:x-amz-algorithme-client-cryptage côté serveur
 a| 
s3:PutObject
 a| 
Compare au `sse-customer-algorithm` ou au étendu à `copy-source-sse-customer-algorithm` partir de l'en-tête de requête dans les requêtes PutObject, CopyObject, CreateMultipartUpload, UploadPart, UploadPartCopy et CompleteMultipartUpload.
