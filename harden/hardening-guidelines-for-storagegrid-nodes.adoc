...
== Modifier le mot de passe BMC de l'appliance

Vous pouvez modifier le mot de passe BMC d'un appareil pour limiter l'accès et améliorer la sécurité du système.

. Dans Grid Manager, accédez à *Nœuds* > *_grid node_* > *Paramètres BMC*.
. Dans la section Changer le mot de passe, saisissez un nouveau mot de passe BMC de 8 à 16 caractères dans *Nouveau mot de passe*, puis saisissez-le à nouveau dans *Confirmer le nouveau mot de passe*.
. Sélectionnez *Enregistrer*.


Vous pouvez également changer le mot de passe BMC à l'aide de l' `PUT /bmc-configuration/root-password`API.



[[control-remote-access-through-usb-portscontrol-external-usb-port-access-on-appliances]]
== Contrôlez l'accès aux ports USB externes des appliances

Vous pouvez activer les ports USB des appliances pour permettre au support technique d'accéder aux ports à des fins de diagnostic et de dépannage. Si vous n'avez pas besoin d'accéder aux ports USB, désactivez les ports pour des raisons de sécurité. Les ports USB sont activés par défaut après l'installation initiale ou une mise à niveau.

* Dans Grid Manager, accédez à *Configuration* > *Sécurité* > *Paramètres de sécurité* > *Appareils* > *Accès aux ports USB* pour activer ou désactiver les ports USB :
+
** Cochez la case *Activer les ports USB* et enregistrez pour activer les ports.
+
Si la case USB a été précédemment décochée et enregistrée, cochez la case *Activer les ports USB* et enregistrez pour activer de nouveau les ports USB.

** Décochez la case *Activer les ports USB* et enregistrez pour désactiver les ports.
...
* N'autorisez pas les clients non fiables à se connecter directement aux nœuds de stockage. Utilisez un point de terminaison d'équilibrage de charge servi par un nœud de passerelle ou un équilibreur de charge tiers.
