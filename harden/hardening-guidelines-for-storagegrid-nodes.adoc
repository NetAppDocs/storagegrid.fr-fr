...
* Pour les appliances sans disques SED, FDE ou FIPS, utilisez un serveur de gestion des clés (KMS) à  https://docs.netapp.com/us-en/storagegrid-appliances/installconfig/optional-enabling-node-encryption.html#enable-node-encryption["Activez et configurez le chiffrement des nœuds logiciels StorageGRID"^].
